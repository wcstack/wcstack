...
**🌐 [wcstack.github.io](https://wcstack.github.io)** — ランディングページ・ライブデモ・全パッケージの紹介。
...
AIコーディングエージェント（Claude Code、Codex、Cursor、Copilot など）で開発するなら、まず **[AGENTS.md](./AGENTS.md)** を読ませてください。このリポジトリのベンダー中立な入口で、2つの方向を両方カバーしています。

- **wcstack を使ってアプリを作る** — [wcstack-app スキル](https://github.com/wcstack/wcstack-skill)に `data-wcs` バインディング構文の完全なリファレンスとタグカタログがあります。生成した HTML は `npx @wcstack/lint --errors-only` が exit code `0` になるまで検証・修正のループを回してください。
- **wcstack 本体を改修する** — パッケージ構成とコマンド、生成ファイルの取り扱い、気軽に変えてはいけないプロトコルをまとめてあります。

Claude Code は [CLAUDE.md](./CLAUDE.md)（より詳細なツール別ガイド）を自動で読み込みます。それ以外のエージェントには、AGENTS.md から読み始めるよう指示してください。
...
複数パッケージを使うページには **`wcstack` エントリバンドル**があります。SPA コア（state / router / fetch / storage / autoloader）を自己完結の 1 タグに束ねたもので、1 リクエスト・ハッシュ 1 個がコア全体をカバーします（2.5.0 時点で 316 KB min / 90 KB gzip）。少なくて済むページでは従来どおり個別パッケージが既定です。jsDelivr の `/combine/` で自分で連結してはいけません（minify 済み ESM は連結に耐えません — [docs/sri.ja.md §3.1](docs/sri.ja.md)）:

```html
<script type="module"
        src="https://cdn.jsdelivr.net/npm/wcstack@2.5.0/dist/auto.min.js"
        integrity="sha384-..."></script>
```
