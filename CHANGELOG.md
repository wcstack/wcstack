...
- `@wcstack/state`: an element that fails to initialize no longer hangs the page silently. `connectedCallback` had one unguarded `await`, so a throw from anywhere inside initialization skipped every promise settlement: `connectedCallbackPromise` and `initializePromise` stayed pending forever with no diagnostic — a page that simply never rendered, and a `renderToString()` / `mount()` that never returned. Such a failure is now reported once with `console.error` and rejects `connectedCallbackPromise` with the original error (the declaration's own message, not a wrapper), while `initializePromise` still resolves so one element's mistake does not drag the rest of the page's bindings down with it. This covers everything initialization can throw: every `$` declaration validator (`$recursion`, `$commandTokens`, `$eventTokens`, `$on`, `$streams`, `$listKeys`, `$watch`), all four state sources (an unsupported `src` extension, a broken `json` attribute, an inner `<script type="module">` that fails, an external module that fails), the `enable-ssr` data merge, a DCC setup failure, a `bind-component` wiring error (a `<wcs-state>` that is not a direct child of a custom element, a conflicting `state`/`src`/`json` source, an unavailable `CustomElementRegistry` for the host, a missing host property, or a host property that is not an object), and the v2 "one `<wcs-state>` per root" rule — where a second root element used to kill the whole page, the first element's page now renders and only the duplicate is refused (that second element stays unregistered but keeps the state it loaded, so remove it; moving a healthy element in the DOM is not a duplicate and is never refused). Detaching an element while its source is still loading is **not** a failure: that connection ends quietly as it always did, and a pooled element that is re-appended initializes, renders, and reports ready. Three consequences come with it: `getBindingsReady(root)` rejects for a root whose state element failed instead of reporting "ready" for a page with no bindings; a `<wcs-state mount="…">` volume that was already waiting for that root when it failed settles with its own report instead of waiting forever (that report is the end of the line for those volumes — an orphaned volume does not re-graft itself when a corrected root is connected later, and its mount slot stays reserved for as long as the page is loaded, so fix the root and reload the page); and `setInitialState()` on an element that already failed throws a diagnostic telling you to replace the element, instead of silently doing nothing. Configuration errors that keep the rest of the page working still resolve `connectedCallbackPromise` as before: `name=`, a plain (unwired) Light DOM `bind-component`, a mount host with no state tree on its root, and a mount-scope setup error. One class stays completely silent and is left unchanged here: a `<wcs-state mount="…">` whose own setup fails before its slot is reserved (a malformed mount path, `mount` combined with `bind-component`, or a mount path already reserved on that root) resolves all three promises and reports nothing at all — no rejection, no `console.error`. ([#257](https://github.com/wcstack/wcstack/issues/257))
