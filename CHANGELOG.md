...
- `@wcstack/state`: **recursive paths** — `$recursion: { "nodes.*": "children.*" }` declares where a tree's shape repeats, and `**` stands for "however deep this is". One getter covers every depth (`get "nodes.**.total"()`, with `this["nodes.**.value"]` and an index-omitted `$getAll("nodes.**.children.*.total")` bound to the depth being evaluated), `$getAll(path, [])` unions every depth (depth-first, pre-order, ascending index), `$setAll(path, [], value)` broadcasts one value to every depth, and each depth's accessor is materialized the first time it is read. `**` never reaches the engine: `PathInfo`, the dependency graph, `$1`…`$n`, `$resolve` and the list diff only ever see ordinary fixed-arity paths, and passing `**` to anything that does not interpret it is `wcs/recursion-unsupported`. This version takes exactly one self-recursive anchor per state, a tree as input (a shared child array is `wcs/recursion-shared-list`, a cycle `wcs/recursion-cycle`), 128 wildcard levels (`wcs/recursion-depth-exceeded`), no recursive setters, no mapper / `{ spread: true }` / prefix in a recursive `$setAll`, and no `**` in markup, `$watch` keys or `$resolve`; rendering a tree stays with a self-referential component. The editor and `wcs-validate` report the statically decidable forms under the same `wcs/recursion-*` codes. Example: `packages/state/examples/recursive-tree/`; design: [docs/state-recursive-path-design.md](./docs/state-recursive-path-design.md). **Known limitation**: replacing row objects while keeping their `children` arrays leaves that row's aggregate stale after the next leaf update (a list-identity defect that hand-written multi-level getters share — README "Recursive Paths → The input has to be a tree", [#256](https://github.com/wcstack/wcstack/issues/256)).
...
- `@wcstack/state` recursive paths (unreleased feature, second post-landing review): a write to a recursive getter's **concrete expansion** — `$setAll("nodes.*.children.*.total", [], v)`, `$resolve("nodes.*.total", [i], v)`, `this["nodes.1.total"] = v`, or a path inside the value the getter returns — is now refused at the write entry with `wcs/recursion-readonly`, whether or not that depth has been materialized. Before, an unmaterialized expansion looked like a missing key: the value landed on the node object and was pinned as the getter's cached result, corrupting the aggregates above it; a materialized one was a silent no-op. A `class`-syntax state whose prototype declares the same concrete getter (`get "nodes.*.total"()`) is now rejected as a collision instead of being silently shadowed by the generated accessor. `$setAll("nodes.**.children.length", [], 0)` is `wcs/recursion-structural-write` (it truncated every child list and left the aggregates stale). Re-setting the **same** state object (or one reusing the same `nodes` array) no longer leaves the previous generation's cached aggregates behind — the generated accessors' cache entries are dropped with their dependency edges, so a structural write before the next read is reflected. A bound `**` (`this["nodes.**.value"]`, index-omitted `$getAll`) read through a plain getter that a recursive getter calls now reports `wcs/recursion-context` instead of a raw `ListIndex not found` or a silently wrong "this depth × every row" value: the depth is taken from the innermost frame only, the same frame the row index comes from. A binding under an object-returning recursive getter (`textContent: nodes.*.stats.count`) no longer gets a false `wcs/binding-path-missing`. `$getAll("nodes.**.value", null)` is a `wcs/recursion-getall-form` diagnostic rather than a `TypeError`, and a mis-spelled anchor with a prefix reports `wcs/recursion-anchor` first (same order as the linter). `$listKeys: { "nodes.**.children": … }` is refused (`wcs/recursion-unsupported`) instead of being accepted and never applied; a non-array `indexes` on a plain `$getAll` is a diagnostic too. The read-only check also folds index spellings in API path arguments (`$setAll("nodes.1.total", [], v)`, `$resolve("nodes.0.children.0.total", [], v)`), which bypass the set trap's normalization and used to land on the raw node object. The same folding now applies to the suffix of a `**` path in a recursive `$setAll`: `nodes.**.children.0` is refused as a child node, `nodes.**.children.0.children` as a child list and `nodes.**.children.0.total` as the getter, instead of replacing children and throwing a raw `TypeError` part-way. `$trackDependency("nodes.**.value")` is `wcs/recursion-unsupported` instead of silently registering a dependency that never fires. A `**` getter whose suffix names the structure (`get "nodes.**.children"()`, `.children.*`, `.children.length`, or the object on the way to a multi-segment repeat's list) is refused when the declaration is read — it would hide the real child list at every depth. Re-setting the same state object also drops the cached results of `**` getters whose suffix has its own wildcard (`get "nodes.**.tags.*.up"()`), and a re-set whose new `$recursion` is invalid no longer forgets the previous generation before throwing.
