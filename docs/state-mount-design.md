...
- **状態**: 2026-09-01 起草。**論点整理・初稿**。§0 の決定レコードは提案であって合意ではない（著者判断待ちは「要確認」で明示）。実装前。**次期メジャー（v2.0.0）の目玉機能**。手順は [state-mount-impl-plan.md](./state-mount-impl-plan.md)。
...
| **D4** | コンポーネントの解決規則 | **R1: own data key ＝ 私有、getter / メソッド ＝ chroot 評価、それ以外 ＝ ツリー**。私有キーがマウント先のキーを隠すときは**バインド時に warn**（＋ lint）。代替 R2〜R4 は §4-3 | **要確認** |
...
| **D8** | コンポーネント内からの絶対パス | **v2.0 では不可**（chroot は厳格）。コンポーネントが要るものはホストがマウントする。`/` 接頭辞による絶対参照は **2.x の候補**として構文上の余地だけ残す（§2-3） | **要確認** |
...
| **D11** | ルートの存在 | **ルート `<wcs-state>` は必須**（空でよい）。ボリュームだけのページは暗黙ルートを作らず **throw** する（§4-8） | **要確認** |
...
1. **D4（R1 か R4 か）** — 既存の mapped コンポーネントの既定値をどう扱うか。著者判断
2. **D8** — 絶対参照を 2.0 に入れるか。入れるなら「マウント表が唯一の結合点」の原則を README で緩める必要がある
3. **D11** — ボリュームだけのページに暗黙ルートを許すか。許すなら「要素の無いルート」の保持者（rootNode ごとの非要素ホルダ）が要る
...
6. **`mount` の語** — `@wcstack/testing` の `mount()` と衝突する。改名しない提案だが、`at` / `path` も候補
