...
| 読み込み済みのボリューム要素**自身**への `setInitialState()` 再 set | throw（#268 — 接ぎ木はロード完了時にデータをルートの木へ一度だけ複製するので、要素の state を入れ直してもページに届かない。旧挙動は無言の no-op。接ぎ木に失敗したボリュームも同じ。差し替えはルートのマウントパスの下へ書く。実装注記 2026-09-15） |
