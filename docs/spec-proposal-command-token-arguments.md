...
| websocket-chat/state | `this.$command.wsSend.emit(JSON.stringify({ ... }))` | 文字列のペイロード |
| users-crud (packages/fetch/examples) | `state.$command.refreshList.emit()` | 引数なし（0個） |
...
- **examples（E2E 実証）**: state-cross-tab-todo（オブジェクト引数）/ websocket-chat/state（文字列引数）/ users-crud（0 引数）/ README 例（2 位置引数）が実ブラウザで動作。
