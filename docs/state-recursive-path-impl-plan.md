...
| X2 | 配列を行から行へ付け替えると描画と `$getAll` が食い違う。→ [#256](https://github.com/wcstack/wcstack/issues/256)（**修理済み**: 台帳は 1 本の配列につき行集合 1 組のまま。行がぶら下がる親が**退役している**ときだけ、行の identity を保って新しい親へ付け替える（`reparentListIndex`）。退役の signal は差分の `deleteIndexSet`、復活の signal は同じ差分の `newIndexes`（印は差分ごとに付け直すので、消えない印は残らない）。付け替えは一方通行ではなく、**外した行が戻ってきたら**持ち主を返す（行集合は最初にその配列を展開した親＝home を憶えている）。戻ってきたかどうかは **ListIndex の identity ではなく行オブジェクト（リスト要素）の identity** で見る —— 行を戻す普通のやり方（新しい配列に同じ行を並べ直す）は差分が ListIndex を作り直すので、ListIndex の identity では永久に「戻ってこない」。main と一致するのは同じ配列インスタンスで戻した綴りだけで、新しい配列で戻すと main は両方の行が凍る（branch は戻した行が追従する）。行オブジェクトごと作り直して戻した場合は別の行なので持ち主は戻らない。「新親と一致しない `parentListIndex` の行は作り直す」案も「台帳を (親, 配列) の組でキーする」案も実測で却下 —— 前者は描画行を重複させ、後者は**生きた共有**（1 本の配列を 2 つの生きた親が指す形）で親ごとに別々の絶対アドレスを作り、片方へ書いた値がもう片方から永久に見えなくなる。生きた共有の意味論は main のまま） |
