...
4. S3 は 3 片（stream / recursion・dcc・watch / scopes）をサンドボックスで実装し終えた（設計案 §8-2 の実装記録: 全テスト 3,668 件成功、core → 機能の辺 41 → **14**、受け口は 12 種、読みのコストはばらつきの内側、full の `auto` は +1.3 KB gzip — 要件 D19 の判断待ち）。D19 は推奨 (a) で決まり（要件 §6）、S4 は 8 片で完了した（ボリューム §8-3、`$streams` / `$watch` §8-4、`$scan` §8-5、bind-component とマウント §8-6、`$recursion` と残り import・ルート初期化失敗の着地 §8-7、DCC の接続 §8-8）。`State.ts` は 1,683 → 1,220 行、機能 import は 24 → 1 本（型のみ）で値の import は 0 本。読み書き・接続の hot path の辺は 0 本。S5 の第 1 片（install と SSR 以外の 5 辺、設計案 §8-9）と第 2 片（SSR の分離 ＝ H8、§8-10）で core → 機能の辺は 17 → 12 → **10 で、すべて入口の辺**（install 8・`exports → ssr/Ssr`・`registerComponents → components/State`）。循環は 31 → 28 モジュール。第 3 片（エントリ分割、§8-11）で `installFeatures` と `@wcstack/state/core` を作り、**core 単体 43,779 B gzip**（機能の差分は scopes +15.4 / temporal +8.3 / recursion +4.8 / ssr +3.1 / devtools +1.9 KB、全部入り 73,504 B。[split-entry-sizes.json](./research/state-next/split-entry-sizes.json)）。core の実測はスタブ化ビルドの上限 44.0 KB とほぼ同じで、配線の切り出しでは縮まなかった（A2 の 35 KB には §5 の残り 3 段が要る）。単一 core チャンクは成立（どの機能エントリにも core のモジュールが入っていない）。分割配信のチャンク代は +13%。辺は監査の分類を直して 62 → **6**。同日に**製品へ移植した**（設計案 §8-12）: `exports` の `./core` / `./features/*` / `./define`、本体 rollup の分割ビルド、CI の門 2 つ（core の再同梱なし・core エントリの閉包サイズ）、README と SRI の文書。`features/formats`（D16 のフィルタ登録簿）も同日に実装した（設計案 §8-13。core 単体 43,779 → 42,705 B gzip）。残るのは A2 に効く §5 の 2 段（`BindingSession` の一本化・診断の dev ビルド化）。結合監査は `--pkg <sandbox>` で試作を測れる。
...
node scripts/research/s4FailedRootPatch.mjs <sandbox2>/packages/state         # S4 第 7 片（同）
node scripts/research/s4DccLifecyclePatch.mjs <sandbox2>/packages/state       # S4 第 8 片（設計案 §8-8）
node scripts/research/s5CoreEdgesPatch.mjs <sandbox2>/packages/state          # S5 第 1 片（設計案 §8-9）
node scripts/research/s5SsrSplitPatch.mjs <sandbox2>/packages/state           # S5 第 2 片（設計案 §8-10。SSR を src/ssr/ へ）
node scripts/research/s5SplitEntriesPatch.mjs <sandbox2>/packages/state       # S5 第 3 片（設計案 §8-11。エントリ分割）— 14 本は冪等で、この順に当てる
node scripts/research/measureSplitEntries.mjs <sandbox2>/packages/state       # split-entry-sizes.json（単一ファイル束ね・dist-split の閉包・core の帰属）
node scripts/audit-state-tech-coupling.mjs --pkg <sandbox2>/packages/state   # coupling-proto.json（片ごとの写しは coupling-proto-s3slice{2,3}.json、S4 は coupling-proto-s4volume / -s4 / -s4final / -s4dcc.json、S5 は -s5edges.json / -s5ssr.json / -s5entries.json）
