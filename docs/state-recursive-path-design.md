...
**同じ配列インスタンスの共有（Phase A 実測 → #256 で修理済み）。** 台帳 [`listIndexesByList`](../packages/state/src/list/listIndexesByList.ts) は現在 **(親, 配列)** の組をキーにする。Phase A の時点では**配列の identity** だけをキーにしていて親を持たず、同じ `children` 配列に 2 つの親から到達できると ListIndex の親が**先に走査した親に固定**され、行 getter が親の値を読むと別名化した値を返し（実測 `[109,109]`、正しくは `[109,209]`）、`$setAll` の mapper 形が**二重適用**されていた。組をキーにしたことで、行の文脈（親を読む getter・行ごとの mapper 適用）は正しくなった。**データの共有そのものは残る** —— 1 本の配列を 2 つの親が指していれば、ブロードキャストの書き込み件数はスロット数ではなく到達経路の数になる。走査が共有を拒否する規則（D12）は変えていない。
...
> **訂正（Phase E 実装・2026-09-10）**: 上の「参照比較 1 回」は誤りだった。台帳（`listIndexesByList`）は当時リスト配列の identity だけをキーにしていて親を持たなかったので、行オブジェクトを作り直すふつうのイミュータブル更新（`nodes.map(n => ({...n}))` は `children` を参照ごと引き継ぐ）で親が入れ替わっただけの正当な木を「共有」と誤認する。実装は**走査そのもの**で判定する —— 1 回の走査の中で同じ配列インスタンスに 2 度到達したら共有（祖先の経路上なら循環）で、台帳の親は見ない（実装計画 §5・`recursion/walk.ts`）。**追記（#256）**: 台帳のキーは (親, 配列) の組になったが、走査で判定する結論は変わらない —— 組キーでは行の親が常に「いま降りてきた親」と一致するので、そもそも判定材料にならない。
