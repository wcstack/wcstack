...
| **D12** | 共有配列の扱い | 拒否する。条件は「DAG」ではなく**同じ配列インスタンスが 2 つ以上の親から到達可能**。検出は参照比較 1 回（§7-2）。循環は同じ述語で捕まる。 |
...
- **`$129` が無言で undefined になる。** `INDEX_BY_INDEX_NAME` は `$1..$128` しか持たず、`traps/get.ts` は表引きに失敗すると通常のプロパティ解決へ落ちる。`$128` は正しく 0 を返すので、境界のすぐ外側だけが診断ゼロで壊れる。
