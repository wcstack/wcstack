...
上記サブゲート 1〜4 はそこで G1〜G4 として推奨付きで展開され、段階分割・受け入れ条件・
却下案まで降ろしてある。

**(d) は 2026-07-31 に不採用と決裁した**（同書 §0-1）。主因は粒度の不一致 —
lane が直列化するのは `$on` ハンドラの実行だが、wcstack のイディオムは
「ノードが非同期を所有し、state はプロパティを小突くだけ」であり、ハンドラは同期で
即終了するため lane に掴むところが無い。ハンドラ所有に寄せるには `emit()` の戻り値を
実質的に仕様面へ昇格させる必要があり、本書冒頭の MUST NOT に抵触する。
**したがって決定ゲート 1 は現時点で (a) I/O タグ宣言のまま据え置きとする。**
(b) / (c) は未評価であり、選択肢としては残る。

なお同書の検討過程から、lane と独立に価値がある 2 点は切り出して実施済み:
`$on` / `onXxx:` ハンドラの reject 捕捉（`packages/state/src/event/captureHandlerRejection.ts`）と、
名前が実装より多くを約束していた `setLoopContextAsync` の削除。
