...
決めた（2026-09-21、第 2 回。要件 §6）:

- **R4 の一本化の形**: 決着済み。R2・R3 の後では一本化の前提が成り立たず、台帳への出入りだけを畳んだ（§6-1）。
- **D24**: プール事前生成の opt-in 属性は 3.0 では作らない。生成の費用を初回描画の前へ動かすだけで総量は減らず、使わない行の分は無駄になる。要望が出たら 3.x で非破壊に足せる（R5 の候補 1・3 は §7-1 で実装済み）。
- **D25**: 残る 3.0 KB/行 の削減は 3.0 の後に回す（R3 で −18%、受け入れ基準は要求していない）。
- **D22**: A2 は 3.0 の core の実測 43.2 KB gzip を基準に改めた。R4 はほぼ効かず（§6-1）、診断の分離（配線設計 §8-14）の後で core は 43.2 KB。**35 KB は、この構造のまま（proxy ＋ 依存グラフ ＋ 二重の初期同期）では届かない** — 届かせるのは D1 の (c)（全面新実装）の判断で、それは 3.0 の後の研究に回した。
