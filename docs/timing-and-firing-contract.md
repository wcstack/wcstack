...
Shell は constructor（= upgrade 時）で自分自身の `*-changed` / `:error` イベントに反映リスナーを登録し、以後解除しない。upgrade 後に `addEventListener` した利用者リスナーからは**常に反映済みの states が見える**（リスナー登録順による）。upgrade 前に登録されたリスナーは反映前に走り得る — この順序は保証しない。
