...
- **Scheduler（enqueue の単一関門）**: 現状 `updater.enqueueAbsoluteAddress` は無条件 push。ここを唯一の enqueue 関門にし、A1-1 同値ガードを正規装備。set→GraphCore でダーティ集合確定→Scheduler が microtask 境界で1回 flush の単方向フローに整流。ループガードは `MAX_DEPENDENCY_DEPTH`（現状 stack depth のみ）を「同一 flush 内 visited 集合」と組み合わせて**値レベルの不動点収束保証**に格上げ。
  - **A1-2 computed 同値短絡はここに属する**（A1 でなく A4）。実測（§A1-2 ★）で「per-set eager 短絡は coalescing を壊し純損」と判明したため、**flush 境界で（flush あたり1回）dirty な getter を再計算→前値と比較→同値なら下流を dirty にしない**、というスケジューリングとして実装する。

#### ★ A4 着手調査の確定結論（2026-06-27・A3 オラクル整備の上で）

A4 の核心（flush 境界 computed 同値短絡）の正しい実装可能性を、A3 オラクルを敷いた上で詰めた。**結論: 正しい A4 は「三色/遅延検証型の反応性コア書き直し」を要する**。安直な実装は2つとも成立しない:

1. **eager（per-set）短絡** → 純損（§A1-2 ★・−の +44〜51%）。microtask coalescing が既に下流再計算を flush 1回に畳んでいるのに、毎 set で getter を再計算するため。
2. **walkDependency を flush へ遅延**（短絡を flush で1回だけ走らせる案） → 性能は出るが **read-after-write 一貫性を壊す**。現行は **set 時に依存を dirty 化**するため、同一 sync ブロック内で set 直後に computed を読むと最新値が返る（`a3.characterization.test.ts` の `read-after-write` が pin: `s.n=5; s.d===50; s.dd===51`）。walk を flush に遅延すると set 時に dirty 化されず、その読みが **stale** になる。

→ 両立させるには「**読みが検証をトリガし（一貫性維持）、再計算は epoch あたり1回に畳む**」三色マーキング（Reactively/Solid 系）が必要で、これは state の cache/getByAddress/伝播エンジンの**根本的な置換**（＝[[signals-state-design]] の `reactive.ts` がやっていること）。**path-addressing の表層を保ったまま反応性エンジンだけを三色化する**作業で、1475 テストの中核を作り直すことになる。

**判定: A4（正しい computed 同値短絡）は予定通りゲートに留める。** 理由 = (a) 利得は性能審が「複数階層束縛の深い安定チェーン限定・実アプリで稀」と判定済みで narrow、(b) コストは反応性コアの三色書き直し＝高リスク、(c) A4 の本来の正当化は「性能」でなく「proxy⇄dependency⇄list の癒着の実害（バグ/拡張困難）」であり、それは未だ示されていない。**A3 オラクル（characterization test）は敷けたので、将来 A4 に踏み込む際の番人は用意できた**が、踏み込む judgement は癒着の実害提示を待つ。実験コード（`_bench.computedShortCircuit` + `walkDependency` の短絡ブロック）は調査記録として既定OFFで残置（`_bench.ts` は調査後に撤去済み）。
