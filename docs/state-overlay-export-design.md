...
### 7-1. replace1k のソース比較

前回の **17.90 → 39.85 ms は性能退行の判断材料から撤回**する。コミット済み dist と新規ビルドの比較で、実行順やウォームアップの影響も分離できていなかった。再計測用の [replace1k-compare.mjs](../e2e/bench/replace1k-compare.mjs) は以下を固定する。

- 機能追加前 `de487346` とレビュー修正後 `4abb5ed9` のソースを `git archive` でそれぞれ一時ディレクトリへ展開し、同じ `packages/state/node_modules` で `npm run build`。インストール・依存更新・作業ツリーの dist 差し替えはしない。
- 同一 HTML / buildData / CSS、seed 固定のデータ生成、Chromium 149.0.7827.55、1280×800、headless、CPU throttle 1。各セッションで新しいブラウザプロセスを起動し、`replace1k` だけを測定する。他のベンチやビルドは同時実行しない。
- ペアの実行順を AB / BA で交互にする。サンプルごとに 2 回の requestAnimationFrame を待ち、直前の描画と計測区間を分離する。強制 GC は行わない。
- `performance.now()` でプログラムからの click 開始〜MutationObserver による DOM 更新完了を測る。**レイアウト・paint・Playwright 通信時間は含まない**。計測直後、次のタスクへ移る前に全 1,000 行の ID とラベルが state と一致することも検証する。
- ウォームアップ値を含む全サンプル、各セッションの中央値、依存バージョン、ソース commit、成果物 SHA-256 を保存する。外れたセッションも除外しない。統計の独立単位は個々の反復ではなくセッションのペアとする。

第1比較は 8 ペア、各セッション 40 回ウォームアップ＋40 回採用（各版 320 サンプル）。セッション中央値の中央値は **14.60 / 14.55 ms**（前 / 後）。ペア比の幾何平均差は **+3.92%**、ペア単位 bootstrap の参考 95% 区間は **−3.61〜+15.30%**。後半の変動を含めると ±2% の範囲には絞れない。生データ: [replace1k-source-comparison.json](../e2e/bench/results/replace1k-source-comparison.json)。

確認比較は **16 ペア・32 セッション、各 60 回ウォームアップ＋60 回採用（各版 960 サンプル）**。両ビルドの成果物 SHA-256 と依存関係は第1比較と一致した。全ウォームアップ・採用サンプルで全行検証が成功。生データ（時刻、前半・後半中央値も含む）: [replace1k-source-confirmation.json](../e2e/bench/results/replace1k-source-confirmation.json)。

| 確認比較の集計 | 結果 |
|---|---|
| 機能追加前: セッション中央値の中央値 | 24.675 ms |
| レビュー修正後: セッション中央値の中央値 | 24.975 ms |
| 上記 2 値の比 | +1.22% |
| 同じペア内の後 / 前の比を幾何平均した差 | +3.27% |
| ペア単位 bootstrap の参考 95% 区間 | −5.98〜+12.29% |

中央値同士の比とペア比の幾何平均は異なる集計であり、95% 区間は後者に対するもの。**約 2 倍の遅延は両比較で再現しなかった。一方、実行時の変動が大きく、±2% 以内という同等性も確認できない**。都合の悪いペアを落としたり、単純な +1.22% だけを使って E12 合格と扱ったりしない。機械の負荷・動作状態の影響と実装差の小さな影響を、この測定だけで分離したとは主張しない。

再現コマンド（`e2e/` から、比較対象のソースを両方ビルドする）:

```sh
node bench/replace1k-compare.mjs --before de487346 --after 4abb5ed9 --pairs 16 --warmup 60 --samples 60 --out bench/results/replace1k-source-confirmation.json
```

### 7-2. 初回レビュー時の参考記録（2026-09-07）
...
|---|---|---|
| jsfb create1k / clear10k（中央値 ms、参考値） | 38.00 / 103.90 | 39.25 / 98.35 |
...
**E12 全体の ±2% 条件は確認できていない**。replace1k の判断には §7-1 のソース比較を使う。その他の指標もビルド条件を揃えた比較と反復測定が必要で、性能劣化なしとは記載しない。この plain ベンチは公開索引の O(N²) 懸念も検証しない。
