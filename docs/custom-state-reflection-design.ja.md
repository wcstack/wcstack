...
- **関連**: [async-io-node-guidelines.ja.md](async-io-node-guidelines.ja.md)（§0 不変条件・§4.2 派生 getter）、[timing-and-firing-contract.ja.md](timing-and-firing-contract.ja.md)
- **English**: [custom-state-reflection-design.md](custom-state-reflection-design.md)
...
2. **timing-and-firing-contract.ja.md**: 横断節（§3 と同格）として §3.5 の 4 契約を追記。以後の新規ノードは tag-design doc に反映状態マップを1表含めることを guidelines §1 のチェックリストに追加
