...
- **状態**: **採択後、Phase 2 を保留 — 追試 E1 も不成立**（2026-09-19）。§0 の優先順位と決定レコードは合意済み（2026-09-18）。実装計画の Phase 0（番人と基準試験）と Phase 1（`ITreePath` 改名・lift の 1 関数化）は main に着地した（PR #291・#292）。Spike S は合意済みの判定規則（実装計画 §5-2）の**規則 5 に該当**し（§5-6）、著者が選んだ追試 E1（節点を経由しない flat 形）も**不成立**（§5-7 — 要素の段がページの半分で +1.5〜2ns のレベルに乗る）。本書の推奨は §11 の **B**（案 A を閉じ、Phase 0 の番人を恒久策とし、main-tp を単独の最適化として切り出す）。未検証の形は §11-4（採るなら新しい spike として事前登録）。**著者判断待ち**。
