...
- **状態**: 調査メモ（非規範）。ここでの検討は「実装した場合にぶつかる壁」の洗い出しであり、設計決定ではない。実装着手時は改めて `docs/<name>-tag-design.md` を起草すること（[async-io-node-guidelines.ja.md](./async-io-node-guidelines.ja.md) §1 MUST）
