...
- **状態**: 2026-08-27。**Phase 0〜4 完了・main 着地済み**（PR#186〜#192）。残るのは**リリースのみ**（末尾の「リリース時の作業」・ユーザー操作）。設計の正本は [i18n-design.md](./i18n-design.md)（以下「設計書」）。本書はその §13 を着手可能なタスク粒度・受け入れ条件・完了条件に展開した手順書。**注記（2026-09-06）**: Phase 4 の「完了」は再スコープ後の姿に対してであり、原受け入れ条件 P14 / S8 は満たしていない（§5 の各行と §9 に記録）。
...
| ID | 内容 |
|---|---|
| **T3-0** | **完了（2026-08-27）**。後から DOM に入ったノードがバインドされない件（T3-0a / T3-0b は同一原因）。C（失敗を loud に）を PR#190 で、B（binder プロトコル）を PR#191 で実施。正本は [binder-protocol-design.md](./binder-protocol-design.md) |
...
**両方とも実施済み（2026-08-27）。** C が PR#190、B が PR#191。**`<wcs-route>` の中のバインドも `<wcs-head>` の翻訳された `<title>` も動く。** デモは `<wcs-head>` を戻し、ルートごとに翻訳されたタイトルを出している。警告は「state を読み込んでいないページ」向けの診断として残っている。実装で外れたタイミングの前提 3 件は [binder-protocol-design.md](./binder-protocol-design.md) §9。
...
| ID | 内容 |
|---|---|
| **T4-1** | **見送り（2026-08-27 判定・2026-09-06 に「不可」から訂正）**。`@i18n` バインドのパスを辞書のキー集合と突合するには、検証器がモジュールを解決する必要がある。vscode-wcs の解析器は設計として **AST を使わず `<wcs-state>` のインラインスクリプトを正規表現で見る**もので、`src=` の追跡機構を持たない — **実ブロッカーはこの道具の欠落だけ**である。当初併記した「辞書は実行時ロケールで決まる動的 import の先にあるので `t` の由来は静的に決まらない」は**言い過ぎだった**: 候補モジュールの集合は head スニペットの対応ロケール宣言から列挙でき、T4-2 が守る不変条件（言語間キー集合の一致）の下では任意の 1 ロケールがキー集合を決める ＝ **検査は決定可能**。「不可」と記録すると設計自身の不変条件の下で決定可能な検査を将来に向けて閉じてしまうため、「**検証器にモジュール解決を足す独立作業として未着手**」と記録し直す（設計書 §12 の追記も同じ訂正） |
| **T4-2** | **完了（2026-08-27）**。`examples/router-i18n/check-catalogs.mjs`。fallback の deep merge で埋まる訳漏れは実行時に見えないので**ここが唯一の検出点**。単体スクリプトにしたのは検証器にモジュール解決が無いため（T4-1 と同じ**道具の欠落**。「静的に決まらない」からではない — 2026-09-06 訂正）。**複数形グループは `Intl.PluralRules` に各ロケールが実際に使うカテゴリを尋ねて比較する** — 素朴なキー突合は `ja` に `count.one` が無いことを誤検出し、誤検出する検査は無視されるようになる |
...
- **本 Phase の「完了」は再スコープ後の姿に対してである（2026-09-06 明記）**。原受け入れ条件 P14（lint と IDE の両方で報告）と S8（解析不能の報告）は満たしていない。マトリクスと実態が黙って並存していた状態を解消するため、§5 の該当行に再スコープを記録した
...
| **P14** | 言語間キー集合の不一致が lint と IDE の両方で報告される — **未達・再スコープ（2026-09-06）**: 報告するのは `check-catalogs.mjs`（単体スクリプト）のみで、lint にも IDE にも入っていない。原条件は T4-1 再開（検証器へのモジュール解決の追加）の受け入れ条件として残す | 4 |
...
| **S8** | `import` を辿れない辞書構成 → 「解析不能」を報告 — **担い手消滅（2026-09-06）**: 担うはずだった検証器統合（T4-1）を見送ったため、この報告を出す機構がそもそも無い。`check-catalogs.mjs` 自体は import 失敗で非ゼロ終了する（黙って通りはしない）。T4-1 再開時に復活させる | 4 |
...
## 9. 2026-09-06 レビュー反映（着地後の指摘 8 件）

外部レビューの指摘 8 件を検証し、**8 件すべて実質的に正しい**と判定して反映した。設計判断そのもの（D6 / D9 / D14）は覆していない — 覆すべき実害がまだ無いため — が、根拠の誤り・記録の欠落・実穴を修理した。

| # | 指摘 | 判定と処置 |
|---|---|---|
| 1 | P14 / S8 が未達のまま Phase 4 完了になっている | **正しい**。マトリクスと実態の黙った並存を解消 — §5 の P14 / S8 行に再スコープを記録し、冒頭の状態行と Phase 4 DoD に明記 |
| 2 | T4-1 の「静的に決まらない」は言い過ぎ | **正しい**。キー集合一致の不変条件の下では 1 ロケールが集合を決める ＝ 決定可能。実ブロッカーは検証器のモジュール解決欠落のみ。T4-1 / T4-2 / 設計書 §12 / チェッカーと README の説明文を訂正し、「不可」→「未着手（決定可能）」へ |
| 3 | `<base href>` の税が「制約 1 つ」より重い | **正しい**。ページ内アンカー（§9-1-1 と同型の静かな壊れ方）・SVG フラグメント参照・csp.md の相対 `src` 推奨との不整合を設計書 **§9-1-4** に列挙。csp.md（英日）と demo の `<base>` コメント・README にも注記 |
| 4 | basename を `<base>` 経由で渡す必然性が薄い（代替が未記録） | **記録欠落として正しい**。router が `<html lang>` を読む案と intercept のリンク単位オプトアウト（SvelteKit / React Router / Nuxt の prior art）を設計書 **§9-1-5** に記録。D9 は維持、再検討トリガを定義 |
| 5 | D14 の本番フォールバックが §9-2 の判断と非整合 | **緊張は実在**。設計書 §4-3 に i18next / Fluent の prior art と併せて記録。維持理由（`t.typo` は state からはただの missing path で、キー描画には i18n 専用機構か core 変更が要る）と再検討トリガを明記 |
| 6 | 「辞書に getter が 1 つも無い」をテストが守っていない | **正しい ＋ 実穴を発見**。旧 `mergeAndFreeze` は片側にしか無いサブツリーを**参照のまま**返しており、その枝は凍結もされず getter が descriptor ごと生存できた。`i18n/merge.js` に切り出して修理し、`e2e/tests/router-i18n.spec.ts` の 1 本（getter 平坦化・片側サブツリーの凍結複製・参照非共有）で §4-1 / §12 の 2 節を固定 |
| 7 | D6 と MF2 移行のあいだの緊張が未記録 | **正しい**。設計書 §7 に追記。規範を 1 つ追加: プレースホルダはデータのまま、適用はアプリ内 1 関数（demo は `format.js` の `fmt()` に集約し、getter 内の `.replace()` を廃止） |
| 8 | head スニペットの localStorage が不変条件連鎖の単一障害点 | **正しい**（実装は既に try/catch 済み・文書が欠落）。設計書 §8-1 に明記し、throw する storage の下でロケールが確定することを e2e の tripwire で固定 |

検証: `check-catalogs.mjs` 挙動不変（意図的な 1 件のみ報告）・`e2e/tests/router-i18n.spec.ts` 13/13 green（既存 11 ＋ 新規 2）。パッケージの src には一切触れていないため、リリース作業への影響は無い。
