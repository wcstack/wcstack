...
- **新エンジンで変わったこと**（承認済みの簡素化）
  - 行の監視は自分でリストを同期する。`for:` も `$listKeys` も要らない。現行は、そのようなリストでは添字付きパスへの書き込みも `ListIndex not found` で失敗する（#324）。
...
- **意図した差分を 2 つ宣言した**:
  - 行の getter のリストで、`for` で描いていないリストの行のパスへ書くと、現行は `ListIndex not found` で投げる（#324）。
...
- ゴールデン: 4 シナリオを足した。
  ...
  - 誤りのコード 7 つ: 6 つは現行と同じ。展開への代入は、新エンジンが契約どおり `[wcs/recursion-readonly]` を出す。現行は `for` で描いていないリストの行のパスで `ListIndex not found` を投げ（#324）、検査まで届かない（意図した差分として宣言）。
...
### 残りの片付け（2026-09-26・コミット `4955ab52`）
...
### core の余裕づくり: エラー文の番号化（2026-09-26）

core は 19,984B gzip で、上限（20,000B）まで 16B しか無かった。

**測ったこと**（`dist/core.min.js` の文字列リテラルを TypeScript のパーサで抜き出し、置き換えて gzip）
- エラー文は 114 片、生で 3.6KB。
- 文面をすべて番号に置き換える: −930B。
- `[wcs/…]` のコード名を残し、説明文だけを消す: −1,071B。
- 効くのは説明文を core の外に出すことで、コードを整数にすることではない。15 種のコード名は繰り返し出てくるので、gzip がすでにほぼ消している。
- 実行時の debug フラグでは、文字列が配信物に残るのでバイトは減らない。「debug モード」は診断の後付けにする（全部入りの `auto` は入れている）。

**方式**（著者承認: コード名＋番号の形、診断の後付けを debug モードとする）
- `src/messages.ts` に `const enum M`（ビルドで数値に展開される。名前は配信物に残らない）。
  - 百の位がコード: 1xx `binding-syntax`、2xx `template-syntax`、3xx `binding-path-missing`、…、15xx `spread-no-bindable`。1〜99 はコード無し。
  - 番号は追加だけにする。ページで読んだ番号の意味が、版をまたいでも変わらない。
- 後付けの無いページの出力: `[@wcstack/state] [wcs/binding-syntax] #108 "b("`。値は、文字列を JSON の引用符付きで、それ以外を `String` で並べる。console に出すもの（バインディングの失敗、drain の打ち切り、トークンの購読者の失敗、命名の上限）も同じ形。
- 診断の後付けを入れると、以前と同じ文面になり、その後に従来どおりの助言（did-you-mean・直し方・lint への誘導）が続く。
  - 文面は `src/diagnostics/messages.ts` の `SENTENCES`。`Record<M, …>` なので、文面の無い番号は型エラーになる。
  - core の受け口に `render` を 1 つ足した。
- 文章のまま core に残したもの: 後付けの無いページで出会う壁。`[wcs/feature-not-installed]` と、formats の無いページの書式フィルタ（「installFormats() で入れる」）。
- 後付け（recursion・temporal・scopes など）の文面は、それぞれの後付けの中にあるので変えていない。
- 付随する変化: 接頭辞の無かった `throw new Error` の 13 か所（readonly、`$eqIndex`、`$setAll`、`$commandTokens`／`$eventTokens` の宣言、`$on`）にも `[@wcstack/state]` が付く（段 8 の「接頭辞の統一」どおり）。
- 内部のメソッド `Engine.fail`（バインディングの失敗）と紛れないよう、投げる関数の名前は `raise` にした。

**テスト**
- 文面を確かめていたパーサとフィルタのテスト（5 ファイル）は、ファイルの先頭で診断を入れる。文面は診断の持ち物になったため。
- element・protocols の 5 件は、後付け無しで流れるファイルなので、core の出力（番号）で確かめる。
- 新しく `messages.test.ts`（8 件）: core だけの形、値の並べ方、console の形、壁は文章のまま、番号の重複なし、百の位にコードの枠がある、すべての番号に文面がある（余分も無い）。
- `diagnostics.test.ts` に 3 件: 以前と同じ文面、文面の後に助言、console の文面。
- テスト 824 件（通過 823・スキップ 1）。

**サイズ（gzip）**
- core 19,984 → **19,000B**（−984B）。上限まで 1,000B。
- 診断 3,250 → 4,649B（+1,399B）。全部入りの `auto` 38,077 → 38,543B（+466B）。
  - 移した文面より増えるのは、番号から文面を作る関数の表と、呼び出しごとの値の配列の分。

**性能**（全部入りの `auto`、直前のコミット `bc4ae3e0` のビルドと同じ回で交互に 4 周・各 24 サンプル、それを 4 回。`addons/messages-targets-ab{,2,3,4}/`）
- 投げない経路は変わらない（`throw new Error(…)` が `raise(…)` の呼び出しになっただけ）。
- 各回とも、周の中で先に測った方がウォーム 1,000 行で 0.5〜1.2ms 速かった（順序の効果）。順序を 2 回ずつ入れ替えて合わせると、差は揺れの範囲。
  - ウォーム 1,000 行: 6.55ms 対 6.45ms（各 96 サンプルの中央値）。
  - コールド 10,000 行: 76.90ms 対 76.85ms。

**残り**
- 番号の一覧を利用者が引ける場所。state-next が現行を置き換えるときに、README か docs に表を置く。lint／VS Code 拡張で番号を解読させてもよい。

### 現行の未解決 Issue の影響（2026-09-26）

#2 を除く未解決 Issue 6 件（#258、#319〜#323）が、新エンジンにも起きるかを調べた。

**方法**
- 各 Issue の再現手順をそのまま `__tests__/issues.test.ts` に起こし、Issue の「期待」を確かめる。
- 同じ手順を Chromium で、現行 3.3.0 と state-next の両方に流した（`docs/research/state-engine/issues/`）。現行ではどれも Issue に書いたとおりのエラーで再現したので、確かめ方そのものも正しい。

**結果**

| Issue | 内容 | state-next |
|---|---|---|
| #319 | 初期値を要素から受け取る wc-bindable メンバーを for の行に置く（出力専用・`#init=element`） | 影響なし |
| #320 | 同じリストを 2 つの for で描き、片方を if で消して戻す | 影響なし |
| #321 | コンポーネントのメソッドが私有キーに書く | 影響なし |
| #322 | 行の中のコンポーネントの getter で `$getAll` | 影響なし |
| #323 | 行の中のコンポーネントから `this["items.0.v"]` に書く | 影響なし |
| #258 | SSR のハイドレーション後、行の getter が葉の更新に追従しない（残り 4 つの穴） | 入れ子の for の内側の行・行の中の mustache・if の中の for は影響なし。**行の中の bind-component の子は同じ穴があった**（下で直した） |

**直した穴: SSR で、行の中に置いたコンポーネント**
- 起きる条件: 行の中のコンポーネント（`bind-component`）のクラスが、ハイドレーションより先に定義されている場合（Chromium でも同じ）。後から定義される場合は動いていた。
- 症状: コンポーネントが独立した木になり、ホストの `state.d` が届かない。`{{ d }}` は `binding-path-missing` で失敗する。
- 原因:
  - サーバの出力では、行の要素から `data-wcs` が外れる（行は計画の複製）。
  - クライアントでは、コンポーネントの `<wcs-state bind-component>` が、ホストが行を引き取る前に接続する。結線を待つ手がかり（要素の `data-wcs`）が無いので、待たずに独立した木として始まる。
- 直し方（`src/scopes/component.ts` だけ。core は変えていない）:
  - サーバで描くとき、ホストが行の中のカスタム要素に束ねた先頭の名前を `data-wcs-wired` に残す。`data-wcs` がその名前を書いている要素（ページの階層）には付けない。
  - コンポーネントは、`data-wcs` と同じく `data-wcs-wired` が自分の名前を挙げていれば、結線を待つ。
  - クライアントのホストが結線を登録するとき（引き取り）に、`data-wcs-wired` を外す。ハイドレーションの後の DOM には残らない。
- `issues.test.ts` の該当のテストは、既知の穴として `it.fails` にしていたものを、通るテストにした。

**新しく起票した Issue: #324**
- `for` で描いていないリストの行を、添字のパス（`this["items.0.v"]`）で読み書きすると、`ListIndex not found` で投げる。読みも書きも失敗し、書き込みは入らない。README の「Direct index access」の約束に反する。
- 最上位のリスト、入れ子のリスト（外側だけ描く・どちらも描かない）のどれでも起きる（`listindex.mjs`）。
- 新エンジンは影響なし（どの形でも読み書きできる）。
- 適合シナリオと本書では「#319 と同系統」と書いていたが、#319 は別の問題（wc-bindable の初期同期）だった。#324 に直した。

**その他**
- happy-dom では、行の中のコンポーネントのクラスを「描画の後に」定義すると、SSR でなくても失敗した。Chromium では現行・state-next とも動くので、happy-dom の、カスタム要素の昇格と `whenDefined` の順序の違いによる。

**確かめたこと**
- テスト 839 件（通過 838・スキップ 1）。`issues.test.ts` 15 件（#319 3・#320 1・#321 2・#322 1・#323 1・#324 2・#258 5）。
- Chromium: `run.mjs` の state-next 14 件（Issue の手順 6・行のコンポーネントの定義の順 2・SSR 6）がすべて通過。現行は Issue の 6 件が再現し、定義の順の 2 件は通った。
- e2e 全体: state-next で 131/131（修正の後）。
- サイズ（gzip）: core 19,000B（変化なし）、scopes 6,619 → 6,725B、全部入りの `auto` 38,543 → 38,662B。
