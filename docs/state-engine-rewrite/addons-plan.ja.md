...
**`@wcstack/server` とのつなぎ方（§5.4 の決定）**
- server 側の変更は要らない。
- `renderToString` の `bootstraps` で state-next を渡す。state-next（`core.js`＋各後付け）は 1 度だけ読み込み、`installFeatures` も 1 度だけ呼ぶ。描画ごとには `bootstrapState()` だけを呼ぶ（現行と同じ使い方）。
- 描画ごとにモジュールを読み直すと、binder など `globalThis` の窓口を最初のインスタンスが持ったままになり、2 回目以降の描画で、ルーターが入れた中身が束ねられない。そのため、読み直してはいけない。
- 組み立て役（`Symbol.for("wcstack.ssr.snapshotBuilder")`）は、install のときに登録する。server はこれを見て、描画を orchestrated にする。

**方式**
- 現行と同じく、サーバの DOM を作り直さずに引き取る（モデルの変更は、`buildBlock` に「複製の代わりに既存のノードを受け取る」受け口を 1 つ足しただけ）。
- **サーバ**（最終パス）
  - ビューの木を上からたどり、各ビューの行・枝を、アンカーの直後の `<!--wcs-[-->`…`<!--wcs-]-->` に移す。行の頭には `<!--wcs-|-->` を付ける。
  - 枝の範囲は、連鎖の最後のアンカーの後ろに、枝の番号付きで置く。こうすると、連鎖のテンプレートが隣り合ったまま残る。
  - ページの階層のアンカーは `<!--wcs-p:ID-->` に替え、元のテンプレートを `<wcs-ssr>` に `template id` で入れる。
  - ページの階層の `{{ }}` は、式を目印に残す（`<!--wcs-t:EXPR-->値<!--wcs-/t-->`）。
  - 隣り合う文字ノードの間と空の文字ノードに印を入れ、HTML を読み直したときに行の構造がそのまま戻るようにする。
- **クライアント**
  - バインディングを付ける前に、文字とテンプレートを元に戻す。
  - ページの階層の範囲は、取り外してアンカーごとに預ける。
  - 行を作るとき、複製の代わりに預けた行を引き取る。
  - 引き取った行の中の入れ子の範囲も、取り外して入れ子のビューに預ける。そのため、計画のノードの位置とサーバの行の構造が一致する。
  - 引き取らなかったもの（クライアントの状態の方が行が少ない等）は捨て、足りない行は複製で作る。
- **ライフサイクル**
  - スナップショットのデータ（自前のデータのキーのみ）は、ページを束ねる前に状態に重ねる。
  - `$connectedCallback` は、サーバでは走り、クライアントでは走らない（現行と同じ）。
  - サーバでは、`$stream` と `$watch` を始めない。
- **版の検査**
  - `<wcs-ssr version>` の major.minor が違えば、警告し、範囲を捨ててクライアントで描く。
  - state-next の出力の形は現行 3.3.0 と互換ではない。混ぜると、この検査で安全に「クライアントで描く」側に倒れる。

**現行の欠陥を構造で解いたこと**
- 入れ子の `for` で、内側の添字が欠けない（行ごとに入れ子の範囲を預けるため）。テストで、内側の行のイベントの添字（`1.0`）を確かめた。
- 複数の行の中の `for`／`if` の目印を、文書の最初の一致に付けてしまうことが無い（アンカーのノードそのものを鍵にするため）。
- テンプレートのフィルタが落ちない（テンプレートの要素をそのまま写すため）。
- UUID の衝突が無い（範囲の目印は位置で読み、ID は `<wcs-ssr>` の中だけで使うため）。

**承認済みの簡素化**
- 古いサーバ向けのインライン snapshot と、プロパティの値の表を落とした。引き取った行のバインディングは、すべて適用される。

**core に足したもの**
- 受け口 `ssrMark`（走査がテンプレートをアンカーに替えたとき、最上位の `{{ }}` を束ねたとき）と `adopt`（`buildBlock`）。`adopt` は、クライアントが引き取っている間（「mounting」から「connected」まで）だけ差し込む。普段の描画では null の判定 1 回だけ。
- インラインの `<script>` を読む `data:` URL の代替。server は `URL.createObjectURL` を外すため。
- 状態の読み込みを 1 マイクロタスク遅らせた。happy-dom は、定義済みの要素を子より先に接続するので、インラインの `<script>` が見つからないまま待ち続けていた。
- core の文字列をさらに 3 つ縮めた（binding の失敗、drain の打ち切り、`$getAll` の文脈）。
- core は 19.99KB gzip（上限まで約 0.01KB）。

**確かめたこと**
- 実ブラウザ: e2e の ssr-router 5 件が、サーバもクライアントも state-next で、すべて通った。
  - 通った内容: サーバ描画、クライアントの引き取り（二重描画なし・目印の撤去・エラーなし）、同じルートでのクエリ遷移、SPA 遷移と深い URL の再読み込み。
  - プロキシは、`/ssr-router/*` を `@wcstack/server`＋state-next で描く。
  - この worktree には、`packages/server` の happy-dom が入っていなかった。そのため、以前の比較では、ssr-router が現行でも落ちていた。
- 単体テスト: `ssr.test.ts` 6 件。
  - サーバの DOM のノードがそのまま残ること（入れ子の `for`・`if/else`・イベントの添字を含む）。
  - 隣り合う文字と空の文字。
  - `$connectedCallback` はサーバだけで走り、得た値がクライアントに渡ること。
  - 版の不一致でクライアントで描き直すこと。
  - 行数の食い違い。
  - サーバでは `$stream` が initial のままで、クライアントで始まること。
- テスト 793 件（通過 792・スキップ 1）。

**サイズ（gzip）**: ssr 2.6KB（現行 4.1KB）。全部入りの `auto` は 35.2KB。
