...
- **状態**: **全 Phase 完了**（2026-07-11・Phase A=0e2c93f / B=f0095f3 / C=413edf4 / D=docs+example+ビルド検証）。フルスイート 172 files / 1624 tests green・lint 0・カバレッジ閾値クリア（`src/stream/*` は 100/100/100/100）・`npm run build` 通過（dist に $streams 収録・d.ts 健全）。残課題は設計書 §11（TS 型公開・signals 逆輸入・DCC 対応・リリース時 minor bump）。設計の正本は [state-streams-design.md](./state-streams-design.md)（以下「設計書」）。本書は設計書 §9 のフェーズ分割を、着手可能なタスク粒度・テスト対応・完了条件に展開した手順書。
