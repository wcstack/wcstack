...
```html
<p data-wcs="text: tokens"></p>
<p data-wcs="text: $streamStatus.tokens"></p>
<p data-wcs="text: $streamError.tokens | isnull(''); if: $streamStatus.tokens | eq('error')"></p>
```
...
1. **binding パス解決**: `getByAddress` の `$command` namespace 分岐（`_getByAddress` 冒頭）の直後に、第 1 セグメントが `$streamStatus` / `$streamError` の場合の分岐を追加。registry から現在値を返す。これで `data-wcs="text: $streamStatus.tokens"` が既存の binding 機構でそのまま解決される（`$command.<name>` を右辺に書ける既存前例と同型）。
...
- 1 tick 内の複数チャンク: fold は**各チャンクに**適用（値は正確に畳まれる）、binding 反映は updater の既存 microtask coalesce で **1 回**。専用機構は追加しない。
...
| S3 | coalesce | 1 tick に N チャンク → fold は N 回・binding flush は 1 回 |
