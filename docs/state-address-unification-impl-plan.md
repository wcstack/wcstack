...
- **状態**: **Phase 0・Phase 1 は main 着地（PR #291・#292・2026-09-18）。Spike S 実施済み（2026-09-19）— 規則 5 が発動し、Phase 2 は保留**。(a2) も (b) も素のパスの読みで床の数倍退行した（§5-3）。次の一手は設計書 §11 の選択肢（追試 E1／案 A を閉じる／P1 の再決定）で、**著者判断待ち**。**ゲートは全て決着済み**（2026-09-18・§2）— 案 A を進める。残る未確定は Spike S の実測で決まる intern の置き場所（G6）だけ。その判定規則（§5-2）は、Phase 0 の A/A 測定を受けて統計量を精密化した版で確定している（F10・著者承認済み）。
