...
| **D8** | 寿命（G6） | 出力は通常の state 値として**切断・再接続を跨いで保持**する。切断中の出来事は畳まない。scan の書き込みは同じ drain の `$watch` の発火より前（D11）なので、`$watch` ハンドラが要素を同期に切断しても、接続中に着地した分は畳んで書かれて残る（§5-6）。`from` は再接続で再開する（`$watch` と同じ二段構え）。`on` は `$on` と同じ購読経路に乗る。切断は event-token の registry を捨てないので、ルート `<wcs-state>` の再接続の後も `$on` と一緒に畳み続ける（[#273](https://github.com/wcstack/wcstack/issues/273) で修正。`scan.on.test.ts`・`integration.tokenReconnect.test.ts`）。`_state` の再セットで宣言を作り直す。SSR では `from` は発火せず出力の実体化だけを行い、`on` は `$on` と同じ扱い。ボリューム（`mount=`）での宣言は raise、マウントされたコンポーネントでは warn（`$streams` / `$watch` と同じ扱い）。 |
...
→ 2026-09-15 修正: `disconnectedCallback` が command-token / event-token の registry を捨てないようにした（`$streams` / `$watch` の registry を保持するのと揃えた）。切断中の発火は両側とも state の解決で止まる（要素のイベントはルートの登録解除で state を引けず、`$command` の emit は `rootNode` の無い `createState` を通れない）。`DEFECT(#273)` は反転し、`integration.tokenReconnect.test.ts` が `$on` と command-token の両方を固定する。
...
→ 2026-09-15 修正（branch `fix/state-row-landing`）:

- **K1**: 提案どおり、`notifyWrite` が `walkDependency` の前に書いたアドレス自身のキャッシュを無効化する（キャッシュ対象の書き込みだけ）。ウォークが新しい配列の台帳を作るようになったので、`integration.recursionKnownDefects.test.ts` の現状固定 (ii)「ネストしたリストへの構造書き込み直後、走査を挟まない `$resolve` が `ListIndexes not found: nodes.*.children` で throw する」も直り、反転した。
- **K2**: `replaced` を一律に捨てる案は採らなかった。`list.*` への要素の書き込み（swap 経路）は差分を通らずに、台帳のその位置へ値を持たない行を差し込み、差し替えられた行は退役しない。一律に捨てると、その位置の唯一の着地を落とす。**差分がリストから外した（退役した）行だけを `gone` にする**（`listIndexesByList.ts` の `isRetiredListIndex`）。
- **`$watch` も同じ選別に載せた**。`$watch` は選別を持たず、実測で K2 のほか、行を書いてからリストを短くした・空にした形で範囲外の読みを評価の失敗として報告し、新しい行で置き換えた・移ってきた行と取り除いた行が同じ位置に並んだ形で、同じ位置に 2 回発火していた。選別を `watch/rowLanding.ts` に切り出して `$scan` と共有する。行の値を書くだけのバッチを重くしないよう、`$watch` は退役した行を含むヒットか、同じ entry・同じ位置のヒットが並ぶ entry だけ位置を引く（リストを読む）。位置を引く読みが throw したら、その entry のヒットは発火せず `evaluate` として 1 回報告する。
- テスト: `scan.from.test.ts` の `DEFECT(#274)` 2 本を反転し、要素の書き込みの 2 本を足した。`watch.rowLanding.test.ts` を新設した。`scan(.ja).md` の「行の着地の既知の穴」は外した。
