...
| **D4** | 参照方法 | テンプレートは `@i18n`、getter は**辞書モジュールを直接 import**。クロス state 読み取りは**要らない**（§5） | 決定 → **v2 で改稿**: `@i18n` は `<wcs-state mount="i18n">` + `i18n.` 接頭辞に置き換え（[state-mount-design.md](./state-mount-design.md) D16。examples/router-i18n は移行済み） |
