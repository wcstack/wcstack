...
**この決定が残す唯一の弱点はテストの置き場である。** `examples/` にはテスト基盤が無いので、`negotiate()` の表駆動テストをどこに置くかが問題になる（§14-2）。
...
| 対象 | 決定 |
|---|---|
| `locale` / `date` / `time` / `datetime` の焼き込み | **修理する**。`const opt` を返り値の関数の内側へ移し、呼び出しごとに解決する |
| `setConfig({locale})` による再描画 | **しない**。グローバル設定は依存グラフに載らない。載せる裏口は作らない |
| 不変条件 | **ロケールの確定（`setConfig`）は、最初のバインドが構築されるより前**。head のスニペット（§8）で行う |
| ライブ切替下でのフィルタ利用 | **該当なし**（D1 を降ろしたため）。フィルタは通常どおり使ってよい |
| `config.locale` の位置づけ | 「このページのロケール」。README に明記する |

**修理の目的は順序事故を消すこと**であって、切替を可能にすることではない。呼び出しごとに解決すれば、`setConfig` が多少遅れても**それ以降に構築されるバインドは正しくなる**（既に構築済みのものは再描画されないので直らない — だから不変条件も併記する）。修理と不変条件はセットで初めて意味を持つ。
...
2. **head スニペットのテスト置き場**（§8-2）— パッケージを作らない決定（D7）の唯一の弱点。`packages/state/__tests__/` に間借りするか、配布物を持つ最小パッケージに翻すか
