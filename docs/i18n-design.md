...
| **D9** | router 連携 | **ロケールは router の basename に置く**（`<base href="/ja/">`）。`/:lang` ルートパラメータは採らない — basename 内のリンクは router に intercept され、言語が変わらないまま何も壊れて見えないため（§9-1）。未対応ロケールの redirect も head スニペットの責務。**`<base>` の税は「制約 1 つ」より重く（§9-1-4）、検討の俎上に無かった代替 2 案を §9-1-5 に記録した**（2026-09-06 追記） | 決定（**2026-08-27 に Phase 0 の実測で反転**） |
...
| **D14** | 訳漏れの扱い | 素のデータなのでバインド確立時に `wcs/binding-path-missing` が出る。描画は空 ＋ 警告 ＋ lint 誘導（§4-1 / §12）。**本番フォールバックを持たないことと §9-2 の判断基準の緊張は §4-3 に記録**（2026-09-06 追記） | 決定 |
...
**§9-2 の判断基準との緊張（2026-09-06 追記・レビュー指摘）**: §9-2 は「`<title>` が空になる」を untranslated より悪いと明確に判定した。同じ基準を当てれば、どの言語にも無いキーの空ボタン・空ラベルも untranslated（キー文字列の描画）より悪いはずで、本節の「空 ＋ 警告」はそれと整合しない — **警告は開発時の診断であり、出荷されたタイポは本番でコンソールを見ないエンドユーザーに空 UI として露出する**。診断（開発時）と最終フォールバック（本番）は別の関心事で、前者があることは後者を不要にしない。prior art も本番フォールバックを持つ側にある（i18next は既定でキー文字列を描画（`parseMissingKeyHandler`）、Fluent は message id に落ちる。どちらも開発時診断と併存している）。

それでも現状の D14 を維持する。理由は正当化の言い換えではなく実装コストの実在で、`t.typo` は state から見て**ただの missing path** であり、i18n のキーだと識別する術が無い。キー文字列を描画するには (a) i18n 専用のフォールバック機構（本節が「要らない」とした当のもの）か、(b) missing path 全般の描画挙動の変更（i18n と無関係な全バインドに波及する core 変更）のどちらかが要る。露出面が deep merge によりタイポだけに絞られること、`check-catalogs`（§12）が静的検出点を持つことを併せて、当面は許容する。**再検討のトリガ**: 出荷後のタイポ空描画が実害として報告されたら、(b) を「missing 時の描画を設定可能にする一般機構」として起案する。なお §9-2 との相違（あちらは訳が存在するのに機構が届かない**無診断**の故障、こちらは警告付きの欠落）は程度の弁明にはなるが、「空はより悪い」という基準そのものへの反論にはならないことを明記しておく。
...
**D6 と MF2 移行のあいだの緊張（2026-09-06 追記・レビュー指摘）**: 独自 ICU を書かない判断（D6）は維持する。ただし現実の代替が getter 内の `.replace("{n}", n)` である以上、放っておくと**書式適用がメッセージごとの手続きコードとしてアプリ側に散る**。MF2 が来たとき最も移行コストが高いのはこの散らばりで、「標準が来たときに移行できない独自資産を作らない」という本節の意図と、実際に積み上がる資産の形が逆を向く。したがって規範を 1 つ足す — **メッセージはプレースホルダ付きのデータ（`{n}` 形。MF2 のプレースホルダ構文と衝突しない）のまま持ち、適用はアプリ内の 1 関数に集約する**。そうしておけば MF2 への移行はその 1 箇所の差し替えで済む。参照実装は `examples/router-i18n/i18n/format.js` の `fmt()`。ヘルパを配布するのではなく形を規範として示すだけであり、D6 は変わらない。
...
**単一障害点の明記（2026-09-06 追記・レビュー指摘）**: 決定順 2 番の読み出し（`localStorage`）は、クッキー／サイトデータを遮断した環境では**プロパティアクセス自体が `SecurityError` を throw する**。この読み出しは `<html lang>` を書くより前にあるので、素通しにするとスニペットがそこで死に、**P1（`<html lang>` が DOM 解析前に確定）から始まる不変条件連鎖 — 辞書モジュール・フィルタ・router basename のすべて — が丸ごと切れる**。storage の読み書きは必ず握って null / no-op に落とすこと（storage が無くても URL が選択を運ぶので、機能は劣化しない）。参照実装は両方向とも握っており、`e2e/tests/router-i18n.spec.ts` が「throw する storage の下でもロケールが確定する」ことを tripwire として固定している。スニペットを写す人がこの try/catch を「無くても動く飾り」と誤読して消すのが、この設計で最も安く起きる事故である。
...
- 税がある: `<base>` はページ上の**あらゆる相対解決**の基準を変える。当初「制約 1 つ（URL をすべて絶対にする）」と書いたが、それは過少記載だった（§9-1-4）
...
#### 9-1-4. `<base>` の税（2026-09-06 追記 — 「制約 1 つ」は過少記載だった）

「ページ上の URL をすべて絶対にする」だけでは済まない。`<base href="/ja/">` は**あらゆる相対解決**の基準を変えるため、URL と意識されないものが巻き込まれる（レビュー指摘 2026-09-06）。

- **ページ内アンカー**。`<a href="#faq">` は base 基準で解決されるので、`/ja/about` に居るとき `/ja/#faq` に飛ぶ。しかも basename の**内側**なので router が intercept する ＝ アンカージャンプのつもりがルート遷移になり、**§9-1-1 が警戒したのと同型の「何も壊れて見えない」壊れ方**がページ内アンカーで再発する。フラグメントも URL であり、`href="/ja/about#faq"` と絶対で書くこと
- **インライン SVG のフラグメント参照**。インライン `style` や属性の `url(#gradient)`・`clip-path` も base 基準で解決される。アイコンやグラデーションを inline SVG で持つページは踏む
- **`<wcs-state src>` の相対指定**。T1-0 で `src` は `document.baseURI` 基準になった（それ自体は正しい修理）ので、[csp.md](./csp.md) が inline state の代替として推奨する `src="./state.js"` は `<base href="/ja/">` の下で `/ja/state.js` を指して 404 になる。**csp.md の推奨と本節の推奨は、相対 `src` のままでは噛み合わない**（csp.md 側にも注記した）。`src="/state.js"` と絶対で書けば両立する

参照実装（`examples/router-i18n/index.html`）の `<base>` 前後のコメントと README の制約記載も同じ内容に更新した。この税の重さは、`<base>` を経由しない代替（§9-1-5）を検討する実質的な理由である。

#### 9-1-5. 検討の俎上に無かった代替 2 案（2026-09-06 追記）

D9 の確定時に比較したのは「`/:lang` ルートパラメータ vs basename」だけで、**basename の受け渡し方式**と**intercept のリンク単位オプトアウト**は選択肢に上がっていなかった。検討して棄却したのか選択肢に無かったのかが読めない状態を残さないため、ここに記録する。**決定（D9）は維持する** — v2 リリース済みの現行形を、実害の報告なしには覆さない。

- **案 1: router が `<html lang>` から basename を導く（オプトイン）**。`basename` 属性を退けた理由は「パース済みかつ upgrade 前という狭い窓」だったが、これは**属性をスクリプトで後から書き込む場合**の話で、router 自身が upgrade 時に `document.documentElement.lang` を読む形には当たらない — `<html lang>` は head スニペットが同期で書いており、module より必ず先にある。この形なら `<base>` が要らず、§9-1-4 の税が全部消える。代償は router がロケールを（オプトインで）知ること 1 点
- **案 2: intercept のリンク単位オプトアウト**。SvelteKit の `data-sveltekit-reload`、React Router の `reloadDocument`、Nuxt の `external` と、主要 router はほぼ全部これを持つ。**§9-1-1 が記述した症状は、まさにこれらの属性が存在する理由**であり、言語リンクにだけ付ければ `/:lang` ルートパラメータ案も成立していた。代償は「付け忘れると §9-1-1 の静かな壊れ方に戻る」＝ 既定安全でないこと。`<base>`/basename 形は付け忘れという概念自体が無い

**再検討のトリガ**: 実アプリで §9-1-4 の事故（ページ内アンカー / SVG 参照）が実際に報告されたら、案 1 を第一候補として起案する。案 2 は i18n と独立に router の escape hatch として検討してよい（価値は i18n に限らない）。
...
**着地の記録と訂正（2026-09-06・レビュー指摘）**: 実装は 2 つ目の検査だけが、lint / IDE ではなく**単体スクリプト**（`examples/router-i18n/check-catalogs.mjs`）として着地した。したがって「lint も IDE も同じ語彙で同じ場所を指せる」という本節の三面共有は、i18n のキー突合についてはまだ成立していない（受け入れ条件 P14 / S8 の再スコープは [i18n-impl-plan.md](./i18n-impl-plan.md) §5 の注記）。1 つ目（バインドパス × 辞書キー）の見送り理由として当初「辞書は実行時ロケールで決まる動的 import の先にあるので `t` の由来は静的に決まらない」を挙げたが、**これは言い過ぎだった** — 候補モジュールの集合は head スニペットの対応ロケール宣言から列挙可能で、しかも 2 つ目の検査が守る不変条件（言語間キー集合の一致）の下では**任意の 1 ロケールがキー集合を決定する**。検査は設計自身の不変条件の下で決定可能であり、実ブロッカーは「検証器（regex ベース）がモジュール解決を持たない」という**道具の欠落だけ**である。「不可」と記録すると決定可能な検査を将来に向けて閉じてしまうため、「**検証器がモジュール解決を得たら戻す検査**」として開いたまま記録する。
