...
| `@wcstack/state/features/formats` | registers the formatting filters (needs a filter registry: the survey's §4.4 question of when filter functions are resolved, placed at binding-plan time) | new |
...
| S3 | H1 read/write boundary hook (27 edges) | 3.0 | none (one check on the hot path) |
| S4 | H3 / H4 lifecycle and declaration hooks, H5 readiness barrier | 3.0 | split entries only (uninstalled throws) |
| S5 | H8 SSR separation, entry split, single core chunk, CI | 3.0 | new entries only |

S1 and S2 can ship early without breaking anything (the same column as N1–N3 in requirements §4). S3 onwards ride the major.
...
- Unifying `BindingSession`'s two paths and the concrete design of the row record, the shared session and the plan-level initial render (survey §10.7, §10.13, §10.14; that they ride the 3.0 vehicle is decided) are outside this design and get their own document.
