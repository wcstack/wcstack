...
- **Two CI gates**: [check-state-split.mjs](../scripts/check-state-split.mjs) (every `features/*` shares the core's chunk and carries no core code of its own — requirement B13) and the core entry added to [check-state-size.mjs](../scripts/check-state-size.mjs) as its **closure** (`core.js` plus the chunks it pulls in), because the entry file alone is a 1 KB shell and would be a meaningless number. The coupling baseline is tightened to `maxCoreToFeatureEdges` 6 and `reachability: { "entries/core.ts": { "noFeatures": true } }`. **Addendum (3.x):** the split gate also fails when a feature entry carries **another feature's** code, and it pins each feature's own gzip against [state-split-baseline.json](../scripts/state-split-baseline.json) (+3 %). Two static imports across that seam had gone unnoticed (`features/scopes` → `watch/watchRuntime`, `features/devtools` → `webComponent/mount.ts`); both now go through a receptacle in `src/bridge/`, a directory that belongs to no feature.
