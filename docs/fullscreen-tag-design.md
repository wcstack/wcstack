...
- **状態**: 実装済み（`@wcstack/fullscreen` として公開済み）。本文書は設計時の論点整理と決定事項の記録であり、コード例・決定事項は実装に追従して更新している（実装の一次情報は `packages/fullscreen/src/`）。
...
`requestFullscreen()`はuser gesture（クリック等のユーザー操作起点のコールスタック）内で呼ばれないと`TypeError`でreject する仕様（WHATWG Fullscreen仕様の fullscreen element ready check がtransient activation欠如時に`TypeError`を返す。`NotAllowedError`ではない）。本ノードはこれを:

- **never-throw**で扱う。`try/catch`（`await`の`reject`を捕捉）し、`error`プロパティに格納する。呼び出し元（Promiseチェーン）は例外で落ちない。
- **README/設計上の明記**: ノード自身はgestureを生成できない。command-token経由でこの`requestFullscreen`commandを呼ぶ場合、その起動元（例: `<wcs-fullscreen data-wcs="command.requestFullscreen: $command.goFs">`が購読し、`<button data-wcs="onclick: $command.goFs">`のクリックハンドラがemitする構成）自体が実際のuser gestureに由来していなければならない。setTimeout内やPromiseの`.then()`の奥で呼ぶと、command-tokenを介していてもgesture文脈が失われrejectされる——これはブラウザの仕様上の制約であり、wcstack側で回避する手段はない。
...
// own property（テストスタブ・意図的なモンキーパッチ）→ Element.prototype
// （プラットフォームの実API）の2段解決。素の el[name] 参照は使わない——
// プロトタイプチェーン全体を辿ると、対象がShell自身（target="self"/子なし省略）
// の場合に WcsFullscreen 自身の requestFullscreen() コマンドメソッドを拾って
// 無限再帰（スタックオーバーフロー）するため。
...
- **サブクラスのシャドウ回避**: 要素側のAPI解決は「own property → `Element.prototype`」の2段で行い、中間のprototype（カスタム要素クラスのメソッド）は意図的に飛ばす。Shell自身が`requestFullscreen()`という同名のコマンドメソッドを持つため、素のプロパティ参照では対象がShell自身のとき（`target="self"`・子なし省略時）にコマンドメソッド自身を拾って無限再帰する。標準名・レガシー名とも対称に同じ解決を通す。
...
async requestFullscreen(element: Element | null): Promise<void> {
  ...
  if (!element) {
    // target セレクタが未解決（要素が見つからない）。「API 非対応」とは
    // 別のメッセージにする — さもないと打ち間違えたセレクタがAPI非対応の
    // 誤診断として利用者に見えてしまう。
  ...
  try {
    ...
    if (gen !== this._gen) return; // stale
    this._setError(e); // TypeError（gesture外呼び出し）等
  }
}
```
...
- **スコープの注意（§2.1との非対称）**: `active`の判定が「自分の解決済みtarget」単位であるのに対し、`exitFullscreen()`とそのno-op判定は**document全体**が単位（ネイティブ`document.exitFullscreen()`がdocumentグローバルAPIであるため）。複数インスタンス下では、自分のtargetがfullscreenでないインスタンスから呼んでも、別インスタンスのtargetがfullscreen中ならそれを解除する。プラットフォームの挙動をそのまま透過する設計であり、README「複数インスタンス」節に明記する。

```typescript
async exitFullscreen(): Promise<void> {
  // no-op 判定は世代の繰り上げより前に行う: 何もしない呼び出しが進行中の
  // requestFullscreen() を追い越す（stale化させて error を握り潰す）ことを防ぐ。
...
- **autoTrigger**: 標準搭載しない。`requestFullscreen()`はgesture文脈が必須なため、`data-fullscreentarget`のようなクリック起動ショートカットを提供しても、そのショートカット自体がクリックイベントハンドラ内で実行される限りgesture文脈は保たれる（クリックイベントリスナー内の同期呼び出しである限り問題ない）。ただし本ノードの最有力な起動経路はcommand-tokenであり（`<wcs-fullscreen>`側に`command.requestFullscreen: $command.<token>`を貼り、ボタン側の`onclick: $command.<token>`から起動する運用が主眼）、`autoTrigger.ts`相当の専用属性は初版では見送る。将来的にトリガー属性の需要が出れば`worker`/`notification`のパターンを流用して追加できる。
...
- 観点:
  ...
  - gesture外呼び出しを模した`reject`（`TypeError`。WHATWG仕様のtransient-activationチェックによるもので`NotAllowedError`ではない）でnever-throw・`error`に格納されること。
  ...
  - target未解決（`null`）の場合、unsupported環境と同じ`error`ではなく別メッセージ（`"Fullscreen target could not be resolved."`）へ落ちること。
...
| §9 autoTrigger | 初版では無し。command-token（`<wcs-fullscreen>`側の`command.requestFullscreen: $command.<token>` + ボタン側の`onclick: $command.<token>`）が主経路 |
...
4. example: 画像ギャラリーの「全画面表示」ボタン（`<wcs-fullscreen>`側`command.requestFullscreen: $command.<token>` + ボタン側`onclick: $command.<token>`）、`hidden: active|not`で終了ボタンを出し分け。
