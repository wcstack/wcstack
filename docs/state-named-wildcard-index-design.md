...
**§9 は §2 とは目的が違う独立提案**（非ループ文脈で軸を束縛するスコープ形）である。
可読性ではなく素のパス構文の合成可能性を扱っており、§2 を採らなくても単独で成立する。
§4.4 の損得計算はそのままでは適用できないので、評価は別に行うこと。
...
#### 2.1.1 綴りの変種 — 軸名パターンの配列（2026-08-19 追記）

同じ表を、軸名を埋め込んだ**パスパターンの配列**でも書ける。

```js
export default {
  $indexNames: [
    "users.$u.profiles.licenses.$l",
  ],
  get "users.*.profiles.licenses.*.registDate"() {
    return fmt(this.$u, this.$l);
  }
...
情報量はマップ形と等価である。`$` で始まるセグメントを見つけ、その手前までを（先行する
`$x` を `*` に置換したうえで）軸パスとして取り出せば、§2.1 のマップがそのまま導出できる
（`users.$u` → `users` に `u`、`users.$u.profiles.licenses.$l` → `users.*.profiles.licenses`
に `l`）。**名前は宣言表の中にしか現れず、正規化後のパス文字列は `*` のまま**なので、
§4.1 の必須条件（名前をパスのアイデンティティに含めない）を満たしたままである。

マップ形に対する利点は 2 つ。

- 1 行で軸の**親子順序**まで読める。マップ形は `"users"` と `"users.*.profiles.licenses"` が
  別エントリになるため、順序関係を読者が組み立て直す必要がある
- **重複宣言を検出できる**。マップ形はオブジェクトリテラルなのでキー重複が黙って後勝ちになる

一方で綴りが増えるぶんの細目が要る（いずれも未決、§8-6）。

- **部分命名**（`"users.*.profiles.licenses.$l"` のように片方だけ命名）を許すか。
  許すのが自然だが、`$` の付け忘れが**有効な宣言として通る**silent な失敗が 1 つ増える（§5.2 と同種）
- **重複エントリ**。前方一致で同じ軸に同じ名前なら冪等、違う名前なら `raiseError`
- **非ワイルドカードセグメントの typo**。宣言時に軸パスの存在を検証する（§6）としても、
  リストパスはバインディングから発見されるため定義時点で確定しているとは限らない

予約キー名は `$indexNames` を維持することを推す。`$listIndexes` は**ランタイムの語彙と衝突する**
（`$resolve(path, indexes)` の引数、`list/listIndexesByList.ts`、`IListIndex` の連鎖）ため
「実行時の添字配列を宣言するもの」と誤読される。加えて `$listKeys`（object）と対になる名前で
値だけ配列になり、shape が揃わない。予約キーは一度公開すると削れない（§4.4）ので、
綴りの良し悪しはここで潰しておく。

**この変種は「採るならこの綴り」であって、採る理由ではない。** 支払うもの（新予約キー、
`$1` との 2 綴り併存、typo の silent undefined、追随先 5 箇所）は 1 つも減らないため、
§4.4 の判断は動かない。
...
2026-08-19 追記: **4 番目の選択肢として §9（スコープ確立形）がある**。軸を束縛したブロックの
中で素のパスを読めるようにする案で、名前と独立に採れるため、この 3 択より先に評価すべき
対象になる。ただし §1.1 の唯一の実需要
（`$getAll("regions.*.states.*.population", [this.$1])`）をこれだけで置き換えるには、
`$getAll` が周囲の軸束縛を尊重して部分縮約する必要がある（§9.5）。そこは §2.4 の
「部分指定」と同じ未決点であり、自動では付いてこない。
...
2. **インデックスオブジェクトの型** — §2.3 は `$u` を「`IListIndex` の read-only proxy」と
   ...
   （2026-08-19: **§9 のブロック形を採れば解消する**。`.prev` が `$u.at(i - 1, cb)` の
   糖衣になり、「パス読み取り器」という第 2 の型が要らなくなる）
3. **親軸の `.prev` が解決不能になるケース** — `$l.prev`（同じ user の 1 つ前の license）は
   ...
   （2026-08-19: **§9 のブロック形なら「内側で `$l` も束縛せよ」と構文が強制する**ため、
   この曖昧さ自体が消える）
...
6. **宣言の綴り** — §2.1 のマップ形か §2.1.1 の配列形か。配列形を採る場合は
   部分命名の可否・重複エントリの扱い・軸パス検証のタイミングが未決
...
## 9. 別提案: スコープ確立形（2026-08-19 追記）

状態は §2 と同じく**検討記録・未決**。ただし **§2 とは目的が違い、§2 を採らなくても
単独で成立する**ため、採否は別に判断する。

### 9.1 何を解こうとしているか

現状 `this["users.*.x"]` が読めるのは「**いま評価中の getter が居る行**」だけである。

- `proxy/methods/getListIndex.ts` の `context` 分岐は、読み取りパスの**最後の**ワイルドカード
  パスを直近アドレスの `indexByWildcardPath` から引く（`proxy/methods/getContextListIndex.ts`）
- そしてその直近アドレスが積まれるのは **getter に入るときだけ**である
  （`proxy/methods/getByAddress.ts` が `stateElement.getterPaths.has(path)` の場合にのみ
  `pushAddress` する）

したがって次の 3 つはすべて `$resolve` / `$getAll` の（パス, 添字配列）二重指定に落ちる。

1. 非ループ文脈（トップレベル getter、行に紐づかないイベントハンドラ）からの行指定
2. **浅い getter からの深い段の読み** — `regions.*.summary` の評価中に
   `regions.*.states.*.population` を読むと、最後のワイルドカードパスが
   `indexByWildcardPath` に無いため `ListIndex not found` になる
3. 他行の読み（隣接項目参照）

§1.1 で見つかった唯一の実需要は 2 番目である。

この提案の主目的は可読性ではない（§7 が性能・正しさを範囲外にしたのは §2 の話である）。
**素のパス構文を、それが宣言された行の外から合成可能にする**という能力側の提案であり、
だから §4.4 の損得計算をそのまま当てはめられない。

### 9.2 綴り

軸に添字を束縛したスコープを張り、その中では素のパスが通る。

```js
$on: {
  onSelect() {
    this.$at({ u: 10 }, () => {
      this["users.*.selected"] = true;         // users.10.selected
      return this["users.*.name"];             // users.10.name
    });
  }
}
```

名前を導入しない場合（§2 を採らない場合）はパスと添字配列で書く。

```js
this.$at("users.*.profiles.licenses.*", [10, 2], () =>
  this["users.*.profiles.licenses.*.amount"]);
```

- コールバックの戻り値がそのまま `$at` の戻り値になる
- 読みだけでなく**書き**も通る（`$resolve(path, indexes, value)` のブロック形）
- 入れ子可。内側の軸は外側の軸が束縛済みであることを要求し、無ければ `raiseError`
  （listIndex の親連鎖が組めないため）
- 真のループ文脈の中でも使え、同じ軸なら**内側が勝つ（shadowing）**。これが `.prev` の実体

名前について: 検討時の綴りは `$u.setIndex(10, cb)` だったが、`setIndex` は変異に読める。
実体は代入ではなくスコープ束縛なので `$u.at(10, cb)` / `this.$at({ u: 10 }, cb)` を推す。

### 9.3 §2 との関係

- **名前は必須ではない。** 名前なし形が成立するので §2 と独立に採れる。逆に §2 を採ると
  `const { $u } = this; $u.at(10, cb)` と書けるようになる、という一方向の相乗りになる
- **§8-2 / §8-3 が解消する。** `.prev` はブロックの糖衣になり第 2 の型が要らなくなり、
  親軸の `.prev` で子軸が未定という曖昧さは「内側で `$l` も束縛せよ」と構文が強制する
- **`$1` の意味は変わらない**（§9.4 の実装形を採る限り）。§8-1 の 2 綴り問題を悪化させない

`$u` の型: 非ループ文脈には「現在の添字」が存在しない。**名前は常に軸ハンドルを返し、
数値としての値（`valueOf` / `Symbol.toPrimitive`）はループ文脈内でのみ定義**（外では
`raiseError`）とすれば型は割れず、§2.3 の「名前はオブジェクト、`$1` は number」と一貫する。

### 9.4 最大の罠 — address stack に積んではいけない

`handler.pushAddress(合成アドレス)` が最短に見えるが、これは**依存追跡を黙って壊す**。

address stack は 2 つの役割を兼ねている。

| 役割 | 参照箇所 |
|---|---|
| 添字の解決文脈 | `proxy/methods/getContextListIndex.ts` |
| **評価中 getter の同一性** | `proxy/methods/checkDependency.ts` と `proxy/traps/get.ts` の `$1` 分岐。どちらも `stateElement.getterPaths.has(lastInfo.path)` で「getter を評価中か」を判定する |

`users.*` のような合成アドレスを積むと `getterPaths.has("users.*")` が false になり、
ブロック内の全読み取りで `addDynamicDependency` も `addIndexDependentGetterPath` も
登録されない。例外は出ず、**再評価されずに DOM が stale になるだけ**である。§5.2 の
silent undefined より重い失敗モードで、`fix/state-index-getter-expansion` で直した
取りこぼし（§7-1 の罠）を今度は仕様として作り込むことになる。

現行 `$resolve` がスタックに触れずに listIndex だけを自前で組み立て、`proxy/apis/resolve.ts`
の冒頭で自分で `addDynamicDependency` しているのは、まさにこの二役を避けるためである。

**正しい形**は、identity stack とは別レイヤの**軸束縛スコープ**
（`wildcardPath → IListIndex` の小さなスタック）を持ち、`getContextListIndex` の解決に
shadow / fallback として足すことである。identity stack には触らない。

- `checkDependency` の動的依存も crossRow 検出もそのまま効く
  （読み取りは従来どおり明示アドレスで `getByAddress` に入る）
- `$1` は getter 自身の行を指したままで、番号が揺れない
- listIndex の親連鎖の組み立ては `proxy/apis/resolve.ts` のループがそのまま再利用できる
- `_getByAddressWithCache` は絶対アドレス（pathInfo + listIndex）でキャッシュするので、
  スコープ内の読みも同じキャッシュに乗る。新しい無効化面は増えない
- 実装量は「`getContextListIndex` に fallback 1 つ + push/pop + API 1 本」に収まる

`list/loopContext.ts` の `LoopContextStack` と `proxy/methods/setLoopContext.ts` は流用しない。
前者は構造レンダリング用で親子のワイルドカード段数を厳格に検証し、後者は多重呼び出しを
`already in loop context` で拒否する。別機構である。

### 9.5 その他の設計論点

| 論点 | 所見 |
|---|---|
| **同期限定** | `proxy/methods/setLoopContext.ts` 冒頭と同じ制約。ただし既存は「await の後に `$1` が raiseError」＝ loud なのに対し、こちらは**外側に別のループ文脈があると黙って別の行を読む**方に倒れる。コールバックが Promise を返したら `raiseError` にすべき |
| **`$getAll` との合成** | §1.1 の実需要（浅い getter からの深い段の集計）をこの形だけで満たすには、`$getAll` が**周囲の軸束縛を尊重して部分縮約する**必要がある。これは §2.4 の「部分指定」と同じ未決点であり、自動では付いてこない |
| **反復形** | `$each("users.*", cb)` のような「行ごとにスコープを張って回す」形は自然な同伴 API だが `$getAll` と役割が重なる。この提案には含めない |
| **範囲外 index** | `proxy/apis/resolve.ts` と同じく index を含むメッセージで `raiseError` する（`docs/state-bind-component-nested-for-design.md` §8.4） |
| **§7-1 との関係** | 絶対 index の `at(10, …)` はオフセット台帳の器にならない。記録できるのは `$u.offset(-1, cb)` 形だけなので、§7-1 に着手するなら API はオフセット形も持つ必要がある |
| **DOM バインド経路** | 無関係。§3 のとおり `apply/getValue.ts` は proxy の `get` を通らない |

### 9.6 未決事項と着手トリガー

1. **API 名**（`$at` / `$scope` / `$with`）と、名前あり形・名前なし形を両方受ける場合の
   引数境界。§8-4 と同じ問題がそのまま再来する
2. **shadowing の可否** — 真のループ文脈の中で同じ軸を再束縛できるか。`.prev` を採るなら
   必要だが、行の中から他行へ**書ける**ことにもなるため、書き込みだけ禁じる余地がある
3. **スコープ内で `$1` が何を指すか** — §9.4 の実装形なら「getter 自身の行」で変わらないが、
   読者は「ブロックの行」と読みうる。ドキュメント上の明示が要る
4. **非ループ文脈での軸ハンドル** — `valueOf` で `raiseError` するか undefined を返すか（§9.3）

着手トリガー（§8 のトリガーとは**別の指標**である点に注意）:

- **非ループ文脈から行を指定して読み書きする箇所**、すなわち `$resolve` / `$getAll` に
  添字配列を渡している箇所が実コードで一定数を超えたとき。`$resolve(` / `$getAll(` の
  grep で §1.1 と同じ方法で数え直せる
- **§7-1 のオフセット追跡に着手するとき**（§9.5）
