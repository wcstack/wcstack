...
**Addendum (2026-09-22, D17 implemented)**: the form changed from an event to a pull (the user's decision). The decision assumed a protocol version bump in 3.0, and there is none: protocol §2 does not bump the version for an additive change, and 3.0 changes nothing else in the protocol. Protocol §4.6 also puts dynamic changes of the dependency graph behind pull, and an event would turn a 10,000-row render into 10,000 events. The source gains `keyedSubscriptions(rootNode)` (optional; the version stays 2), which returns per path the counts of per-row subscriptions, keys and `$eqIndex` list watchers, the last value, and whether the path derives from a getter and fell back to a tracked read (`tracked`). `tracked` is a record the ledger never acts on, written only by the fallback branches. The summary is assembled in `devtools/keyedSubscriptions.ts` (the devtools feature); the core gains only the record and an accessor (split core +82 B, `auto.min.js` +223 B gzip). The `@wcstack/devtools` State pane gains a **Keyed selection** section, checked in a real browser through the playground and `e2e/devtools-smoke.mjs`.
