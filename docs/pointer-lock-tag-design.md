...
- **状態**: 実装済み（`@wcstack/pointer-lock` として公開済み）。本文書は設計時の論点整理と決定事項の記録であり、コード例・決定事項は実装に追従して更新している（実装の一次情報は `packages/pointer-lock/src/`）。
...
このため、設計時点では**実装着手はバッチ内の実需要確認後に行うべき**と位置づけていた（[io-node-batch-implementation-plan.md](./io-node-batch-implementation-plan.md) バッチ1の実装順「Fullscreen → Picture-in-Picture → Pointer Lock」の末尾に対応）。設計自体はFullscreen/PiPと同型で先に確定できたため、本書はその下敷きとして先に用意しておいたものであり、その後実需要が確認されて実装済みとなっている（§7参照）。
...
以下は`PointerLockCore.wcBindable`。`inputs`（`target`）はCoreではなく**Shell側**`WcsPointerLock.wcBindable`が追加する — target解決はShellの責務であり、Core自体はShell固有の入力を持たない（fullscreen/networkと同型）:
...
- `active`は`document.pointerLockElement === 解決済みtarget`の都度比較（fullscreenの`active`と同型）。ただし`detail`は**bare boolean値そのもの**（`{ active }`形式ではない）でgetterを持たない——fullscreenの`{ active }`形状+getterとは異なる決定である。（旧版の本節は誤って`{ active }`形式+getterを規定していたが、実装（bare boolean・getterなし）を正としてこの記述を実装に合わせて更新した。）
...
fullscreenと同型（[fullscreen-tag-design.md §9](./fullscreen-tag-design.md#9-shell属性autotrigger)）。`target`属性のみ、autoTriggerは初版では持たない。`requestPointerLock()`もgesture文脈必須のため、起動経路はcommand-token（`<wcs-pointer-lock>`側の`command.requestPointerLock: $command.<token>` + ボタン側の`onclick: $command.<token>`）が主経路となる。
...
| 論点 | 決定 |
|---|---|
| 優先度 | バッチ内最低（用途がゲーム/canvas・WebGL限定でwcstackの宣言的SPA主眼から外れるため）。実装は実需要確認後に着手し、完了済み |
...
**バッチ内の実装順は最後**（Fullscreen → Picture-in-Picture → Pointer Lock）。想定どおりバッチ内実需要確認後に着手し、以下の手順で実装済み（実装の一次情報は `packages/pointer-lock/src/`）:
...
4. example: canvas/WebGLの視点操作デモ（`<wcs-pointer-lock>`側`command.requestPointerLock: $command.<token>` + ボタン側`onclick: $command.<token>`でロック開始、`Esc`キーでの自動解除は仕様側の挙動なので`pointerlockchange`経由で`active`が追従することを示す）。README内のコード例として反映済み — `examples/`配下の独立デモアプリは未整備（将来課題）。
5. README ja/en（用途がゲーム/描画UI限定である旨、`movementX`/`movementY`は将来課題である旨を明記）。実装済み。
