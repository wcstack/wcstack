...
**実装**: `packages/state/src/_bench.ts`（実験フラグ + カウンタ・既定OFF・非破壊。調査後に撤去済み）+ `setByAddress.ts` 冒頭に同値ガード（primitive 限定・`Object.is` 比較・参照型は素通し）。実マウント（`<wcs-state>` + 300行リスト + getter 依存 `doubled` / `items.*.tax`）で OFF/ON を**交互計測**（JIT 順序バイアス除去・M=20,000・median of 7）。計測は `__tests__/bench.gate0.test.ts`。
...
- **実装**: `walkDependency` に eager 短絡（getter 依存を pop 時に再計算→`Object.is` 同値なら部分木を枝刈り）をフラグ背後（`_bench.ts` `computedShortCircuit`・既定OFF）でプロトタイプ。計測 `__tests__/bench.a1-2.test.ts`。（`_bench.ts` および当該ベンチは調査後に撤去済み）
