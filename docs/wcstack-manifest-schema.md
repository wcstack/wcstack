...
- **application** artifact — the concrete app. Holds `manifestExtensions.wcstack.application` with the **single** `stateSchema` (JSON-Schema subset — one state tree per root in v2; volumes contribute subtrees via `wcs-schema emit --mount=<path>`), filter input/output declarations, and list-context roots. Application binding graphs are checked against the resolved package contracts.

## 3. `manifestExtensions` namespaces

Each namespace carries its own `version` (checked against a supported range) and a `components` (package) map or a `stateSchema`/`filters` (application) payload.
...
- **`wcstack.application`** (application, schemaVersion 2): a single `stateSchema` (the whole tree — volume types live as subtrees at their mount path), `filters[name] = { input, output }`, `listContexts: path[]`. *(schemaVersion 1's `states[name] = { stateSchema }` predates v2's single state tree; readers reject it with a migration hint — regenerate with `wcs-schema emit`.)*
...
1. **Discovery** — *(amended 2026-08-30, [app-testing-and-typescript-impl-plan.md](./app-testing-and-typescript-impl-plan.md) D8; the previous "caller-supplied order" rule could not be honored by the IDE, which has no manifest input at all, and so broke IDE / CLI parity.)* For an HTML document, the application artifact is the **nearest `wcstack.manifest.json`** found by walking up from the HTML file's directory — the same file reader that resolves `<wcs-state src>` is the only input, so the IDE and the CLI discover the same file. Exactly one artifact is taken: a manifest in a parent directory is **not** merged with one in the child. A nearest manifest that is broken is still the result (its diagnostics are reported on the manifest, and the HTML is validated as if no schema were declared); the walk does not continue past it. **Explicit CLI inputs** (`*.manifest.json` arguments) that contain at least one `application` artifact replace discovery wholesale for every HTML input of that run. Discovery order among explicit inputs affects only *diagnostic ordering*, never *which contract wins* (see collision). A `package` artifact named `wcstack.manifest.json` is discovered but contributes no state schema.
...
**Path existence against a declared `stateSchema`** (consumed since 2026-08-30, D6 of the plan above). For a state that has a `stateSchema`, a bound path is checked in this order: (1) any candidate the script/JSON analyzer already derived (methods, getters, `$listKeys`, `$streams` — declarations a schema does not carry) counts as existing; (2) otherwise the path is resolved against the schema with the three-valued resolver: `resolved` → silent, **`nonexistent`** (an ancestor is a definite object that lacks the member) → **`wcs/path-nonexistent` (error)**, `unknown` (a bare `{}`, a dynamic or untyped node) → silent. A state with no `stateSchema` keeps the pre-existing `wcs/binding-path-missing` (warning). `for:` on a path whose schema type is definitely not an array is **`wcs/path-type-mismatch` (error)**. Where the analyzer and the schema both know a path, the schema's type wins (D12). Generators must therefore emit a **bare `{}`** (never `{ "type": "object" }`) for values they cannot describe — `Date`, `Map`, depth cut-offs — or everything below turns into false `nonexistent` errors.
