...
アプリのナビゲーション構造をマークアップで定義する未来を妄想してみる。ネストされたルート、レイアウト、型付きパラメータ — すべてがネイティブなHTML要素として存在する。ルーター設定オブジェクトもJavaScriptの儀式も不要。タグが「どこに何を置くか」を語る。

それが `<wcs-router>`, `<wcs-route>` たちの探求するもの。CDN一発、依存ゼロ、構文はHTML。
