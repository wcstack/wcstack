...
- **Speak / Stop** via command-tokens (`$command.speakNow.emit(text)` / `$command.stopSpeak.emit()`).
...
- `command.speak: $command.speakNow` forwards the emitted text straight to `<wcs-speak>.speak(text)` — the command-token argument-forwarding contract.
