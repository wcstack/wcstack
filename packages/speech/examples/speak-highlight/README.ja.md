...
- **Speak / Stop** を command-token で実行（`$command.speakNow.emit(text)` / `$command.stopSpeak.emit()`）。
...
- `command.speak: $command.speakNow` は emit したテキストをそのまま `<wcs-speak>.speak(text)` へ転送します（command-token の引数転送契約）。
