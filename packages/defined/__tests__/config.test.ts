...
describe("config", () => {
  ...
  it("デフォルト設定が正しい", () => {
    ...
    expect(config.tagNames.defined).toBe("wcs-defined");
  });

  it("tagNames を更新できる", () => {
    setConfig({ tagNames: { defined: "my-defined" } });
    expect(getConfig().tagNames.defined).toBe("my-defined");
  });
  ...
  it("setConfig 後にキャッシュがクリアされる", () => {
    const config1 = getConfig();
    setConfig({ tagNames: { defined: "x-defined" } });
    const config2 = getConfig();
