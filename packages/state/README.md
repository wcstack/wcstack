...
- If the tree already has the key (`user.display` exists as data), the tree wins and the runtime warns once (`wcs/mount-export-shadowed`). Two components exporting the same key on the same instance is a configuration error and throws on read (`wcs/mount-export-ambiguous`).
