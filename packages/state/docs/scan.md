...
`$scan` is a declaration map on the state object, in the same family as `$stream` and `$watch`. Each entry folds the occurrences of a **source** (a state path or an event token) with `fold`, and keeps the result in an **output property**.

It needed a place of its own because neither existing declaration can hold this value:

- `$stream.fold` folds **within one run**. When a dependency changes and the stream restarts, the value goes back to `initial`.
...
- the write was made while an earlier batch was being drained: during binding application (a `$renderedCallback`, or a `$on` handler run by an event that an element dispatched synchronously while its binding was being applied), or inside the `$scan` / `$watch` listener (a `$watch` handler, or another scan writing its output). The ledger is cleared at the end of that listener, before the batch the write belongs to is drained; a write by a listener that runs after it in the same drain — the `$stream` restart writing `initial` — still carries `prev`. A `from` that is the output of a `from` scan therefore always gets `undefined`. An `on` scan writes its output inside the event — outside the drain, unless the event came during binding application — so a `from` that reads it gets `prev` like any other write.

A `$stream` chunk carries `prev` when the chunk is a primitive; an object chunk gets `undefined`. The reset to a primitive `initial` that a restart writes carries the last chunk as `prev`, even when that chunk was an object.
...
- An output name is empty, contains `.` or `*`, starts with `$`, or is inherited from `Object.prototype`. It collides with a getter, a setter, a method (a function-valued property) or a `$stream` name.
...
| Order | Mechanism |
|---|---|
| 1 | `$renderedCallback` (inside binding application) |
| 2 | `$scan` (`from` / `resetOn`) — folds and writes the outputs |
| 3 | `$watch` |
| 4 | the dependency-driven `$stream` restart |

While a `<wcs-view-transition>` accepts the `state` participant, binding application moves to a frame, and the order becomes `$scan` → `$watch` → `$stream` restart → `$renderedCallback` ([timing-and-firing-contract.md](../../../docs/timing-and-firing-contract.md) §4.3).
...
$stream: { pageResult: { args: (s) => s.page, source: loadPage } },
...
- It fires on addresses, without comparing values (the same as `$stream` `args`). A primitive only lands when it actually changes, thanks to the same-value guard; an object path resets even when re-assigned with the same contents. An object path resets only when that object itself is written (`state.filter = { … }`), not when one of its children is (`state["filter.text"] = "b"`); to reset on a child change, list the leaf paths (`resetOn: ["filter.text", "filter.tag"]`) or bump a nonce. A write while the state element is disconnected does not reset.
...
| Reconnect | `from` / `resetOn` fire again. `on` folds again too: like `$on`, its subscription is kept across the disconnect ([#273](https://github.com/wcstack/wcstack/issues/273)). |
...
| fold within one run, where a restart may discard it | `$stream` `fold` |
...
This is the shape of `examples/state-intersect-scroll`. `$stream` owns each page run, `$scan` owns the feed across runs, and `$watch` re-arms the sentinel once the feed has grown.
...
| With `$stream` | Restart wins; deriving `args` from the scan's own output raises |
