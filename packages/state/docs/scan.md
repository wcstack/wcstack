...
- **A wildcard `from`** folds per row. When several rows land in one batch, `acc` is chained through them in ascending index order and the output is written once at the end. For row addresses to land, the list has to be rendered with `for` or declared in `$listKeys` — the same condition as `$watch`. Row landings are narrowed to one per position of the list as it is at the drain: an address whose position no longer exists (the row was written and then the list was shortened in the same job) is not folded, and when one position carries both the row now there and a row that was removed from the list, only the row now there is folded. A position that carries only the address of a removed row is still folded, reading the value now at that position (see [Known gaps in row landings](#known-gaps-in-row-landings)).
...
#### Known gaps in row landings

Two shapes do not land as one fold per changed row. `$watch` sees the same landings. Both are pinned by the `DEFECT(#274)` tests in `scan.from.test.ts` and tracked in [#274](https://github.com/wcstack/wcstack/issues/274).

- **Replacing a nested list with a longer array.** `state.$resolve("groups.*.items", [0], [a, b])` over a one-row list lands only position `0.0`; the row at `0.1` never lands, so it is not folded. The write's dependency walk reads the nested list from the cache before the new array is committed there, sees no change, and expands only the rows of the old array. Replacing the outer list (`state.groups = [...]`) is not affected.
- **Writing a row and then removing it from the middle of the list in the same job.** With `[a, b, c]`, writing `b` and then `state.items = [a, c]` folds `c`'s unchanged value once at position `1`, with `b`'s `prev`. The row that moved into that position does not land, and a position that carries only a removed row is folded by index so that a nested replacement of the same length (which lands through the old row) is not lost.
