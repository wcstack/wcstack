...
行オブジェクトを作り直して `children` 配列を引き継ぐ置換 —— `this.nodes = this.nodes.map(n => ({ ...n }))` —— はふつうの更新で、集計もそのまま追従します。行は **(親, リスト)** の組で同定されるので、新しい行オブジェクトのもとで展開した子リストは自分の行を持ち、旧行に結び付いたままにはなりません（[#256](https://github.com/wcstack/wcstack/issues/256)）。
