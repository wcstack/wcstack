...
`installFeatures([...])` は上の例のとおり `bootstrapState()` の**前**に呼びます。機能は自分のタグ
（`<wcs-ssr>`）を登録することがあり、タグを定義するのは bootstrap だからです。逆順でもエラーには
なりません — 後から install した機能は、すでに bootstrap 済みのレジストリへ自分のタグを定義して
追いつきます — が、宣言どおりの順で呼べば、最初の要素が upgrade する前にすべてのタグが揃います。
...
- **`#ro`**: `state.taxRate#ro: settings.taxRate` なら読めても書けません。ボリュームのコードからの代入・`$setAll`・書きの `$resolve` は `[wcs/mount-readonly]` を投げます（ルート自身は書けます）。
- **静的なパスだけ**: 右辺にワイルドカード（`items.*.x`）や `$` 始まりのパスは使えません（ボリュームにはループ文脈がありません）。フィルタも受けません — 変換は getter で行ってください。形の誤りは `[wcs/mount-path-invalid]` で、ロードの前に報告されます。
