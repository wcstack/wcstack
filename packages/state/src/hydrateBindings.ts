 ...
 * 5. <wcs-ssr> を除去
 */
function cleanupSsrDom(root: Document): void {
  ...
  for (const { comment, uuid } of placeholders) {
    ...
    if (tpl) {
      // <template> の content を保持したまま復元するため、新しい template を構築
      ...
      if (imported.childNodes.length > 0) {
        ...
        // content が空の場合、直接の子ノードをコピー（happy-dom の <template> 内 <template> 挙動対策）
