 ...
 *
 * **台帳の親（`newIndexes[0].parentListIndex`）で判定してはならない。** #256 で台帳の
 * キーが (親, 配列) の組になったいま、行の親は常に「いま降りてきた親」と一致するので
 * 判定材料にならない。組キーより前は、行オブジェクトを作り直すふつうのイミュータブル
 * 更新（`nodes.map(n => ({...n}))` は children を参照ごと引き継ぐ）でも親 ListIndex が
 * 別物になり、正当な木を恒久的に拒否していた。反証レビューが blocking として出した形で、
 * このファイルの前半はその回帰テストである（判定に使えないことは変わらない）。
 *
