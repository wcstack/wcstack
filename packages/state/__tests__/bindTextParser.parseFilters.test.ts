...
describe('parseFilters', () => {
  ...
  it('引数ありのフィルターをパースできること', () => {
    const result = parseFilters(['gt(10)']);
    expect(result.length).toBe(1);
  ...
  it('複数引数のフィルターをパースできること', () => {
    const result = parseFilters(['substr(0,5)']);
    expect(result.length).toBe(1);
  ...
  it('複数のフィルターをパースできること', () => {
    const result = parseFilters(['uc', 'trim', 'slice(2)']);
  ...
  it('閉じ括弧がない場合はエラーになること', () => {
    expect(() => parseFilters(['gt(10'])).toThrow(/missing closing parenthesis/);
  });

  it('開き括弧がない場合はエラーになること', () => {
    expect(() => parseFilters(['gt10)'])).toThrow(/missing opening parenthesis/);
  });
  ...
  it('チェーンしたフィルターが正しく動作すること', () => {
    const result = parseFilters(['trim', 'uc']);
  ...
  it('ダブルクォート内のカンマを正しく扱えること', () => {
    const result = parseFilters(['defaults("Hello, World")']);
  ...
  it('クォート付き引数と通常引数を混在できること', () => {
    const result = parseFilters(['pad(5,"0")']);
