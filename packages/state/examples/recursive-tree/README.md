...
- **The recursive write is the broadcast only** (`wcs/recursion-setall-form`): no mapper and no `spread` — the number of indexes changes with depth — no index omission, and no structural writes (the node itself, its children list, or a child node: `wcs/recursion-structural-write`). Structural changes stay ordinary writes, which is what **+ child** and **−** do from inside one level's own scope.
