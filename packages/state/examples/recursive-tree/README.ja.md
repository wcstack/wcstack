...
- **再帰の書き込みはブロードキャストだけ**（`wcs/recursion-setall-form`）。mapper と `spread` は深さで添字の本数が変わるため不可、添字省略も不可、構造への書き込み（ノード自身・子リストとその `length`・子ノード・反復サブパスが `branch.children.*` のように多段なら子リストへ至る途中のオブジェクト）も不可です（`wcs/recursion-structural-write`）。構造変更は普通の書き込みのままで、**+ child** と **−** がまさにそれを 1 段のスコープの中でやっています。
