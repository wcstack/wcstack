...
| State | On when |
|-------|---------|
| `connected` | `wcs-sse:connected-changed` fires with `true` (cleared on `false`) |
| `loading` | `wcs-sse:loading-changed` fires with `true` (cleared on `false`) |
| `error` | `wcs-sse:error` fires with a non-`null` detail (cleared on `null`) |

```css
wcs-sse:state(connected) ~ .indicator { color: green; }

wcs-sse:state(loading) ~ .spinner { display: block; }
wcs-sse:state(loading) ~ .spinner { display: none; } /* default */
...
selectors never match, but `<wcs-sse>` itself keeps working normally
(graceful degradation, never-throw).

**SSR**: `:state()` cannot be serialized into HTML, so server-rendered markup
never carries these states on first paint (`@wcstack/server` is unaffected).
If you need to style the pre-hydration gap, pair your rule with
`wcs-sse:not(:defined)` instead.
