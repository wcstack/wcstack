...
import { NotificationCore } from "../src/core/NotificationCore";
 ...
 */
describe("NotificationCore の wc-bindable semantics", () => {
  const namesWith = (semantics: string): string[] =>
    NotificationCore.wcBindable.properties
      .filter((p) => p.semantics === semantics)
      .map((p) => p.name)
      .sort();

  it("notification lifecycle の edge は event（同値 dedupe してはならない）", () => {
    expect(namesWith("event")).toEqual(["clicked", "closed", "shown"]);
  });
