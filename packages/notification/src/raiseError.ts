...
// do throw on invalid input). NotificationCore follows a never-throw design and
// surfaces all failures through the `error` / `unsupported` state rather than
...
export function raiseError(message: string): never {
  throw new Error(`[@wcstack/notification] ${message}`);
}
