...
function createShareElement(): WcsShare {
  const el = document.createElement("wcs-share") as WcsShare;
  document.body.appendChild(el);
  return el;
}

describe("Share: CustomStateSet (:state()) reflection", () => {
  afterEach(() => {
    document.body.innerHTML = "";
  });

  it("初期状態は全てオフ（states が空）", () => {
    const el = createShareElement();
    expect(getStates(el)).toEqual(new Set());
  });

  it("wcs-share:loading-changed(true) で loading が on になる", () => {
    const el = createShareElement();
    el.dispatchEvent(new CustomEvent("wcs-share:loading-changed", { detail: true }));
    expect(getStates(el)?.has("loading")).toBe(true);
  });

  it("wcs-share:loading-changed(false) で loading が off に戻る", () => {
    const el = createShareElement();
    el.dispatchEvent(new CustomEvent("wcs-share:loading-changed", { detail: true }));
    el.dispatchEvent(new CustomEvent("wcs-share:loading-changed", { detail: false }));
    expect(getStates(el)?.has("loading")).toBe(false);
  });

  it("wcs-share:cancelled-changed(true) で cancelled が on になる", () => {
    const el = createShareElement();
    el.dispatchEvent(new CustomEvent("wcs-share:cancelled-changed", { detail: true }));
    expect(getStates(el)?.has("cancelled")).toBe(true);
  });

  it("wcs-share:cancelled-changed(false) で cancelled が off に戻る", () => {
    const el = createShareElement();
    el.dispatchEvent(new CustomEvent("wcs-share:cancelled-changed", { detail: true }));
    el.dispatchEvent(new CustomEvent("wcs-share:cancelled-changed", { detail: false }));
    expect(getStates(el)?.has("cancelled")).toBe(false);
  });

  it("wcs-share:error が非nullなら error が on になる", () => {
    const el = createShareElement();
    el.dispatchEvent(new CustomEvent("wcs-share:error", { detail: { message: "boom" } }));
    expect(getStates(el)?.has("error")).toBe(true);
  });

  it("wcs-share:error がnullなら error が off に戻る", () => {
    const el = createShareElement();
    el.dispatchEvent(new CustomEvent("wcs-share:error", { detail: { message: "boom" } }));
    el.dispatchEvent(new CustomEvent("wcs-share:error", { detail: null }));
  ...
  it("attachInternals 不在でも throw しない（debugStates は空配列）", () => {
    ...
    try {
      expect(() => {
        el = document.createElement("wcs-share") as WcsShare;
      }).not.toThrow();
    } finally {
