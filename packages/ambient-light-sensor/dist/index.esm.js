...
const WCS_AMBIENT_LIGHT_SENSOR_ERROR_CODE = {
 ...
 *   sensor-error。
 */
function deriveAmbientLightSensorErrorInfo(name, message) {
    if (name === "unsupported") {
        return { code: WCS_AMBIENT_LIGHT_SENSOR_ERROR_CODE.CapabilityMissing, phase: "probe", recoverable: false, message };
    }
    if (name === "SecurityError" || name === "NotAllowedError") {
        return { code: WCS_AMBIENT_LIGHT_SENSOR_ERROR_CODE.NotAllowed, phase: "start", recoverable: false, message };
    }
    if (name === "NotReadableError") {
        return { code: WCS_AMBIENT_LIGHT_SENSOR_ERROR_CODE.NotReadable, phase: "execute", recoverable: false, message };
    }
    return { code: WCS_AMBIENT_LIGHT_SENSOR_ERROR_CODE.SensorError, phase: "execute", recoverable: false, message };
}
...
class AmbientLightSensorCore extends EventTarget {
    static wcBindable = {
        ...
        version: 1,
        properties: [
            { name: "illuminance", event: "wcs-ambient-light-sensor:reading", semantics: "state", getter: (e) => e.detail.illuminance },
    ...
    _setError(error) {
        ...
        this._commitErrorInfo(error === null ? null : deriveAmbientLightSensorErrorInfo(error.error, error.message));
    ...
    _commitErrorInfo(info) {
        ...
        this._target.dispatchEvent(new CustomEvent("wcs-ambient-light-sensor:error-info-changed", {
            detail: info,
            bubbles: true,
        }));
    }
