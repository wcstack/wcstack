...
describe("AmbientLightSensorCore", () => {
  ...
  describe("errorInfo taxonomy (Phase 6)", () => {
    it("初期状態の errorInfo は null", () => {
      expect(new AmbientLightSensorCore().errorInfo).toBeNull();
    });

    it("errorInfo は wcBindable property(error の直後)として宣言される", () => {
      const names = AmbientLightSensorCore.wcBindable.properties.map((p) => p.name);
    ...
    it("errorInfo は error と同期して遷移し、error より前に error-info-changed が流れる", () => {
      ...
      const core = new AmbientLightSensorCore();
      const order: string[] = [];
      core.addEventListener("wcs-ambient-light-sensor:error-info-changed", () => order.push("errorInfo"));
      core.addEventListener("wcs-ambient-light-sensor:error", () => order.push("error"));
      core.start();
