import { describe, it, expect, afterEach } from "vitest";
import { WcsAmbientLightSensor } from "../src/components/AmbientLightSensor";
import { registerComponents } from "../src/registerComponents";
import { getStates } from "./helpers";

// registerComponents経由でカスタム要素を登録
registerComponents();

function createAmbientLightSensorElement(): WcsAmbientLightSensor {
  const el = document.createElement("wcs-ambient-light-sensor") as WcsAmbientLightSensor;
  document.body.appendChild(el);
  return el;
}

describe("WcsAmbientLightSensor: CustomStateSet (:state()) reflection", () => {
  afterEach(() => {
    document.body.innerHTML = "";
  });

  it("初期状態は全てオフ（states が空）", () => {
    const el = createAmbientLightSensorElement();
    expect(getStates(el)).toEqual(new Set());
  });

  it("wcs-ambient-light-sensor:error が非nullなら error が on になる", () => {
    const el = createAmbientLightSensorElement();
    el.dispatchEvent(new CustomEvent("wcs-ambient-light-sensor:error", {
      detail: { error: "unsupported", message: "AmbientLightSensor is not supported" },
    }));
    expect(getStates(el)?.has("error")).toBe(true);
  });

  it("wcs-ambient-light-sensor:error がnullなら error が off に戻る", () => {
    const el = createAmbientLightSensorElement();
    el.dispatchEvent(new CustomEvent("wcs-ambient-light-sensor:error", {
      detail: { error: "unsupported", message: "AmbientLightSensor is not supported" },
    }));
    el.dispatchEvent(new CustomEvent("wcs-ambient-light-sensor:error", { detail: null }));
    expect(getStates(el)?.has("error")).toBe(false);
  });

  it("attachInternals 不在でも throw しない（debugStates は空配列）", () => {
    const proto = HTMLElement.prototype as any;
    const original = proto.attachInternals;
    delete proto.attachInternals;

    let el!: WcsAmbientLightSensor;
    try {
      expect(() => {
        el = document.createElement("wcs-ambient-light-sensor") as WcsAmbientLightSensor;
      }).not.toThrow();
    } finally {
      proto.attachInternals = original;
    }

    expect(el.debugStates).toEqual([]);
    expect(() => {
      el.dispatchEvent(new CustomEvent("wcs-ambient-light-sensor:error", { detail: { error: "unsupported", message: "" } }));
    }).not.toThrow();
  });

  it("probe が SyntaxError を投げる環境（旧Chromium相当）でも _internals が無効化され動作継続する", () => {
    const proto = HTMLElement.prototype as any;
    const original = proto.attachInternals;
    proto.attachInternals = function (): ElementInternals {
      return {
        states: {
          add: () => { throw new DOMException("Failed to execute 'add' on 'CustomStateSet'", "SyntaxError"); },
          delete: () => {},
          has: () => false,
        },
      } as unknown as ElementInternals;
    };

    let el!: WcsAmbientLightSensor;
    try {
      expect(() => {
        el = document.createElement("wcs-ambient-light-sensor") as WcsAmbientLightSensor;
      }).not.toThrow();
    } finally {
      proto.attachInternals = original;
    }

    expect(el.debugStates).toEqual([]);
    expect(() => {
      el.dispatchEvent(new CustomEvent("wcs-ambient-light-sensor:error", { detail: { error: "unsupported", message: "" } }));
    }).not.toThrow();
  });

  it("debugStates はスナップショットを返す（返り値を変更しても states に影響しない）", () => {
    const el = createAmbientLightSensorElement();
    el.dispatchEvent(new CustomEvent("wcs-ambient-light-sensor:error", {
      detail: { error: "unsupported", message: "AmbientLightSensor is not supported" },
    }));

    const snapshot = el.debugStates;
    snapshot.push("injected");

    expect(el.debugStates).toEqual(["error"]);
    expect(getStates(el)?.has("injected")).toBe(false);
  });

  it("debug-states 属性ありで data-wcs-state-* がトグルされる", () => {
    const el = createAmbientLightSensorElement();
    el.setAttribute("debug-states", "");

    el.dispatchEvent(new CustomEvent("wcs-ambient-light-sensor:error", {
      detail: { error: "unsupported", message: "AmbientLightSensor is not supported" },
    }));
    expect(el.hasAttribute("data-wcs-state-error")).toBe(true);

    el.dispatchEvent(new CustomEvent("wcs-ambient-light-sensor:error", { detail: null }));
    expect(el.hasAttribute("data-wcs-state-error")).toBe(false);
  });

  it("debug-states 属性なしでは data-wcs-state-* が一切書かれない", () => {
    const el = createAmbientLightSensorElement();

    el.dispatchEvent(new CustomEvent("wcs-ambient-light-sensor:error", {
      detail: { error: "unsupported", message: "AmbientLightSensor is not supported" },
    }));

    expect(el.hasAttribute("data-wcs-state-error")).toBe(false);
  });
});
