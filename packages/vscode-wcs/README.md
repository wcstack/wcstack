...
- `data-wcs="textContent: count|` → `gt`, `eq`, `upper`, `trim` ... (all 48 built-in filters)
...
| `wcs/binding-syntax` | What the runtime parser rejects since `@wcstack/state` 3.0 (error): an unterminated quote in filter arguments, a second `#` (`value#ro#wo` — write `value#ro,wo`), a value after `else:`, modifiers or left-side filters on `for` / `if` / `elseif` / `else` / `...`, and an empty filter (`x|`, `x||y`). Judged by the canonical parser (`@wcstack/state/parser`), so lint and the runtime agree; quoted `;` / `|` in filter arguments are not separators |
...
| `wcs/name-alias` | An old name that `@wcstack/state` 3.2 renamed: a filter (`uc` → `upper`, `fix` → `toFixed`, …), a dependency API (`$trackDependency` → `$dependOn`) or a declaration key (`$streams` → `$stream`, `$updatedCallback` → `$renderedCallback`). It works through 3.x and is removed in 4.0; the message suggests the canonical name | ℹ info |
| `wcs/declaration-alias` | A state that declares **both** the old and the canonical key (`$streams` and `$stream`, `$updatedCallback` and `$renderedCallback`) — the runtime refuses to guess which one wins and throws at load time | ❌ error |
| `wcs/declaration-alias-read` | *Reading* a declaration key by its old name (`this.$streams`). Unlike declaring it, the old name cannot stay: normalization copies the key to the canonical name and deletes the old **own** property, so the read is silently `undefined`. Warning when the AST proves the read (an object-literal state — the old name is always an own property there); info when the script cannot be parsed statically (a `class` state, where an old name on the **prototype** is not deleted and still reads today) | ⚠ / ℹ |
...
| `wcs/scan-declaration-invalid` | `$scan` entry the runtime rejects: empty, non-flat or `Object.prototype` output name, output clashing with a getter, setter, method or `$stream` entry, neither or both of `from` / `on`, a `from` / `on` that is not a non-empty string or a non-string `resetOn` element, missing `initial` / `fold`, `on` token not in `$eventTokens`, malformed `from` / `resetOn` path (a `**` there is reported as `wcs/recursion-unsupported`, as the runtime does), `from` reading its own output, `resetOn` with `*`, equal to `from` or under it, or reading a scan output, scans that feed each other through `from`, `$scan` or one of its entries written as an array, a `from` that is a setter without a getter (a `resetOn` may be one), and `$scan` declared in a volume (`mount=`; in a mounted `bind-component` it is a warning) | ❌ error |
| `wcs/scan-source-computed` | `$scan` `from` / `resetOn` that is a getter (or sits under one), or a `from` that is an expansion of a `$recursion` `**` getter (`nodes.*.total`) — folding it would count re-evaluations, not events | ❌ error |
| `wcs/scan-path-missing` | `$scan` `from` / `resetOn` path that does not exist in the state — it silently never folds (or never resets). A `$recursion` expansion counts as existing, as for `$watch` | ⚠ warning |
