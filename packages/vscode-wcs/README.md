...
| `wcs/declaration-alias-read` | *Reading* a declaration key by its old name (`this.$streams`). Unlike declaring it, this does not work even in 3.x: normalization copies the key to the canonical name and deletes the old property, so the read is silently `undefined`. Warning when the read is proven by the AST; info when the script cannot be parsed statically (a `class` state) and the regex fallback found it | ⚠ / ℹ |
