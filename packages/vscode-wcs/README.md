...
| `wcs/scan-declaration-invalid` | `$scan` entry the runtime rejects: empty, non-flat or `Object.prototype` output name, output clashing with a getter, setter, method or `$streams` entry, neither or both of `from` / `on`, a `from` / `on` that is not a non-empty string or a non-string `resetOn` element, missing `initial` / `fold`, `on` token not in `$eventTokens`, malformed `from` / `resetOn` path (a `**` there is reported as `wcs/recursion-unsupported`, as the runtime does), `from` reading its own output, `resetOn` with `*`, equal to `from` or under it, or reading a scan output, scans that feed each other through `from`, `$scan` or one of its entries written as an array, a `from` that is a setter without a getter (a `resetOn` may be one), and `$scan` declared in a volume (`mount=`; in a mounted `bind-component` it is a warning) | ❌ error |
