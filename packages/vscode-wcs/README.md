...
| `wcs/v3-migration` | A form that `@wcstack/state` 3.0 rejects or reads differently, found with the same check the 2.x runtime uses for its `[wcs/v3-migration]` warning: a second `#` (`value#ro#wo`), a value after `else:`, modifiers or filters on a structural directive or spread, `radio#ro:` / `checkbox#ro:`, an unterminated quote in filter arguments, an unquoted `true` / `false` / `null` in `eq` / `ne` / `defaults`. Info, so `--strict` does not fail on it; extra filter arguments stay under `wcs/filter-arity` | ℹ info |
