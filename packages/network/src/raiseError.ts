...
// do throw on invalid input). NetworkCore follows a never-throw design and
// surfaces all failures through the `supported` state rather than throwing, so
...
export function raiseError(message: string): never {
  throw new Error(`[@wcstack/network] ${message}`);
}
