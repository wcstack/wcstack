...
describe("config", () => {
  ...
  it("デフォルト設定が正しい", () => {
    ...
    expect(config.tagNames.audio).toBe("wcs-audio");
  });

  it("tagNames を更新できる", () => {
    setConfig({ tagNames: { audio: "my-audio" } });
    expect(getConfig().tagNames.audio).toBe("my-audio");
  });
  ...
  it("setConfig 後にキャッシュがクリアされる", () => {
    const config1 = getConfig();
    setConfig({ tagNames: { audio: "x-audio" } });
    const config2 = getConfig();
