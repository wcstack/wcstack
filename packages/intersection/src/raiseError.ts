...
// do throw on invalid input). IntersectionCore follows a never-throw design and
// surfaces nothing on invalid options (silent no-op, like the geolocation `error`
// property approach), so it has no synchronous throw path today and does not
...
export function raiseError(message: string): never {
  throw new Error(`[@wcstack/intersection] ${message}`);
}
