...
1. **上書き消去（clobber）**: `value` はこの要素で唯一の*双方向*メンバなので、既定のバインディング authority は `state` です。初期の state→element 適用が state スロットのシード値を `value` に書き込み、write-through 保存が**リロードのたびに永続データを上書き**します。
