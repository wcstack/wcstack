...
| `errorInfo` | `WcsIoErrorInfo \| null` | 直近の失敗の serializable な taxonomy（安定 `code` / `phase` / `recoverable`）、または `null`。追加的で `error` の shape は不変。`wcs-fetch:error-info-changed` を発火 |
...
| ステート | on になる条件 |
|----------|----------------|
| `loading` | `wcs-fetch:loading-changed` が `true` で発火（`false` でクリア） |
| `error` | `wcs-fetch:error` が非 `null` の detail で発火（`null` でクリア） |

```css
wcs-fetch:state(loading) ~ .spinner { display: block; }
wcs-fetch:state(loading) ~ .spinner { display: none; } /* デフォルト */
...
なりますが、`<wcs-fetch>` 自体は通常どおり動作し続けます（graceful degradation・never-throw）。

**SSR:** `:state()` は HTML にシリアライズできないため、サーバーレンダリングされた
マークアップの初期ペイントにはこれらのステートは乗りません（`@wcstack/server` は無改変）。
ハイドレーション前の見た目を制御したい場合は、代わりに `wcs-fetch:not(:defined)` と組み合わせてください。
...
`error` プロパティ／イベントの shape は**不変**で、`errorInfo` は直近の失敗の serializable な
taxonomy（`WcsIoErrorInfo`、shape は [TypeScript 型](#typescript-型)）を投影し、破壊的変更
なしに DevTools やアダプタが失敗を分類できるようにします。

関連する 2 つのシグナルは `FetchCore` の **Core 限定 opt-in getter** です（`wc-bindable` の
`properties` ではないため `data-wcs` のバインド対象ではありません）:
...
> **Note (v1.13 以降):** 自動 fetch は同期実行ではなく microtask に遅延されます。
> 同一 tick 内の複数の入力書き込み（例: `...:` spread が `url` と `manual` を順に
> 書く場合）は最終的な要素状態に対する 1 回の判定に集約され、変化のない `url` の
> 再書き込みでは再 fetch しません。接続時の fetch を待つには
> `connectedCallbackPromise` を使ってください — `appendChild` 直後に `promise` を
> 同期的に読むと、自動 fetch ではなく初期の解決済み Promise を掴みます。明示的な
> トリガ（`fetch()`、`trigger`、`fetch` コマンド）は影響を受けず、即時実行のままです。
...
| `errorInfo` | `WcsIoErrorInfo \| null` | 直近の失敗の serializable な taxonomy（安定 `code` / `phase` / `recoverable`）、または `null` |
...
- **`properties`** — `bind()` がサブスクライブする観測可能な出力（`value`, `loading`, `error`, `status`, `objectURL`, `errorInfo`、および Shell の `trigger`）
...
// serializable な失敗 taxonomy（`errorInfo` プロパティの値）
...
interface WcsFetchCoreValues<T = unknown> {
  ...
  errorInfo: WcsIoErrorInfo | null; // 直近の失敗の serializable な taxonomy。それ以外は null
}
