 ...
 * 実行時ガードとして具体化する。
 *
 * 配置方針 (§5): 共通 abstraction が安定するまで全 I/O package の必須依存にはせず、
 * 各 node が小さな helper を持つ。これは fetch 系ノードのローカルコピーであり、
 * `raiseError.ts` / `protocol/wcBindable.ts` と同じ「コピー配布」方式に従う
 * (ランタイム依存を導入しない)。2 ノード以上が必要とし形が固まった段階で
 * 共有パッケージへ昇格する。
 *
