...
- **Empty url suppresses auto-fetch**: `get "detailFetch.url"()` returns `""` when nothing is selected (initial view, and after a filter clears the selection). `<wcs-fetch>` treats an empty url as "no request" and skips the auto-fetch, so no detail request is issued — the pane stays idle on first load, and after a filter the last loaded card simply stays up. This contract matters: without it the relative `""` would resolve to the page itself and the fetch would try to parse the HTML document as JSON.

  Note that an empty url does not reset the element's outputs, and it must not be papered over by writing `detailFetch.value = null` from state: `<wcs-fetch>` is the sole authority over its own outputs, and nulling a value the mounted detail bindings still read makes them throw. The de-dup guard compares against the last url actually *fetched* (see [the timing contract](../../../../docs/timing-and-firing-contract.md) §1.2), so an empty intermediate step does not re-arm a repeat of the same url either — re-requesting it needs an explicit `fetch()` / `trigger` / command.
