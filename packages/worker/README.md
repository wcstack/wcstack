...
- **Trusted Types.** `new Worker(src)` is a `TrustedScriptURL` sink. Under `require-trusted-types-for 'script'` the `src` — an attribute the page author wrote — is signed by a policy named `wcstack`, so the CSP needs `trusted-types wcstack;` (or your own policy, injected as described in [docs/csp.md](../../docs/csp.md) section 7).
