...
- **Trusted Types。** `new Worker(src)` は `TrustedScriptURL` の sink です。`require-trusted-types-for 'script'` 下では、ページの作者が書いた属性値である `src` を `wcstack` という名前の policy で署名するため、CSP に `trusted-types wcstack;` が必要です（または自前の policy を注入する。詳細は [docs/csp.ja.md](../../docs/csp.ja.md) の §7）。
