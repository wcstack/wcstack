...
`<wcs-resize>` reflects one boolean output state onto its
...
| State | On when |
|-------|---------|
| `observing` | `wcs-resize:observing-changed` fires with `true` (cleared on `false`) |

`width` / `height` / `entry` are **not** reflected — they are continuous
values, not boolean observables (see [design doc](../../docs/custom-state-reflection-design.md) §3.2).

```css
wcs-resize:state(observing) ~ .measuring-badge { display: block; }
wcs-resize:state(observing) ~ .measuring-badge { display: none; } /* default */
```
...
selectors never match, but `<wcs-resize>` itself keeps working normally
(graceful degradation, never-throw).

**SSR**: `:state()` cannot be serialized into HTML, so server-rendered markup
never carries these states on first paint (`@wcstack/server` is unaffected).
If you need to style the pre-hydration gap, pair your rule with
`wcs-resize:not(:defined)` instead.
...
- `el.debugStates` — a **snapshot** array of the currently-on state names
  (e.g. `["observing"]`). It is not part of `wc-bindable` (not a bind target)
  and its shape is not a guaranteed contract — use it for debugging only.
- The `debug-states` attribute (opt-in, default off) mirrors state changes
  onto a `data-wcs-state-observing` attribute on the element, so the Elements
