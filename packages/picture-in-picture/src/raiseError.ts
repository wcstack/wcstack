...
// do throw on invalid input). PipCore follows a never-throw design and
// surfaces all failures through the `error` state rather than throwing, so
...
export function raiseError(message: string): never {
  throw new Error(`[@wcstack/picture-in-picture] ${message}`);
}
