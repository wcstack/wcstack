...
**What a property signal receives.** On each `event`, `signals[name]` / `on(name)` take `getter(event)`; when the descriptor omits `getter`, the wc-bindable default `(e) => e.detail` applies — the same rule as `@wcstack/state`'s two-way bindings and `@wc-bindable/core`, so an element binds identically under every adapter. The **initial seed** is the one place that reads the property (`node[name]`) instead: there is no event to derive from at bind time (SPEC § Initial Sync). Every wcstack I/O node dispatches its property value as `detail`, so the two reads agree; a third-party element whose `detail` differs from the property (`detail: { value }`, or no `detail` at all) needs an explicit `getter` — e.g. `getter: (e) => e.target.value`.
...
Relative to the Shell you give up: element-connected lifecycle (drive the Core's `observe()`/`dispose()` — or its start/stop commands — yourself, e.g. `onCleanup(() => core.dispose())`), attribute-based config, and `:state()` CSS reflection (a Shell/ElementInternals feature). It fits pure-logic nodes (fetch, websocket, timer, defined, raf, …); element-coupled nodes (intersection/resize targets, camera preview, fullscreen) keep earning their Shell. The structural Core surface — `EventTarget` inheritance, self-dispatch default, `static wcBindable`, getter-readable observables, `observe()`/`dispose()`/`ready`, never-throw — is **normative across wcstack IO nodes** ([`docs/async-io-node-guidelines.md`](../../docs/async-io-node-guidelines.md) §3.9) and semver-protected; constructor **config** parameters remain per-package — see each package README's Headless usage section.
