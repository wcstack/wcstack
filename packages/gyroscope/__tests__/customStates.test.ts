...
function createGyroscopeElement(): WcsGyroscope {
  const el = document.createElement("wcs-gyroscope") as WcsGyroscope;
  document.body.appendChild(el);
  return el;
}

describe("Gyroscope: CustomStateSet (:state()) reflection", () => {
  beforeEach(() => {
  ...
  it("初期状態は全てオフ（states が空）", () => {
    const el = createGyroscopeElement();
    expect(getStates(el)).toEqual(new Set());
  });

  it("wcs-gyroscope:error が非nullなら error が on になる", () => {
    const el = createGyroscopeElement();
    el.dispatchEvent(new CustomEvent("wcs-gyroscope:error", {
      detail: { error: "unsupported", message: "Gyroscope is not supported" },
    }));
    expect(getStates(el)?.has("error")).toBe(true);
  });

  it("wcs-gyroscope:error がnullなら error が off に戻る", () => {
    const el = createGyroscopeElement();
    el.dispatchEvent(new CustomEvent("wcs-gyroscope:error", {
      detail: { error: "unsupported", message: "Gyroscope is not supported" },
    }));
    el.dispatchEvent(new CustomEvent("wcs-gyroscope:error", { detail: null }));
  ...
  it("attachInternals 不在でも throw しない（debugStates は空配列）", () => {
    ...
    try {
      expect(() => {
        el = document.createElement("wcs-gyroscope") as WcsGyroscope;
      }).not.toThrow();
    } finally {
