...
describe("config", () => {
  ...
  it("デフォルト設定が正しい", () => {
    ...
    expect(config.triggerAttribute).toBe("data-clipboardtarget");
    expect(config.tagNames.clipboard).toBe("wcs-clipboard");
  });
  ...
  it("triggerAttribute を更新できる", () => {
    setConfig({ triggerAttribute: "data-copy" });
    expect(getConfig().triggerAttribute).toBe("data-copy");
  });

  it("tagNames を更新できる", () => {
    setConfig({ tagNames: { clipboard: "my-clipboard" } });
    const config = getConfig();
    expect(config.tagNames.clipboard).toBe("my-clipboard");
  });
