...
// do throw on invalid input). PointerLockCore follows a never-throw design and
// surfaces all failures through the `error` state rather than throwing, so it
...
export function raiseError(message: string): never {
  throw new Error(`[@wcstack/pointer-lock] ${message}`);
}
