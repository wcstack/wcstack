...
// do throw on invalid input). MidiCore follows a never-throw design and surfaces
// all failures through `permission` / `error` state rather than throwing, so it
...
export function raiseError(message: string): never {
  throw new Error(`[@wcstack/midi] ${message}`);
}
