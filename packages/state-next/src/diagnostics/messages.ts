...
export const SENTENCES: Record<M, Sentence> = {
  ...
  [M.BindTextNoColon]: (t) => `Invalid bindText: "${t}". Missing ':' separator.`,
  [M.StructuralTakesNoModifiers]: (t, keyword) => `"${t}": "${keyword}" takes no modifiers or filters on its left side.`,
  [M.ElseTakesNoValue]: (t) => `"${t}": "else" takes no value.`,
  ...
  [M.LeadingDotNamespace]: (prop) => `"${prop}": a leading "." needs a property that is not a namespace.`,
  [M.UnterminatedQuote]: (quote, args) => `unterminated ${quote} quote in "(${args})".`,
  ...
  [M.FilterTrailing]: (f, trailing) => `"${f}": unexpected "${trailing}" after the filter's closing ")".`,
  [M.FilterEmpty]: (source) => `an empty filter in "${source}".`,
  [M.FilterNameHasModifiersInput]: (name, mods) => `"${name}" is not a filter name: a modifier list "${MODIFIER_SEPARATOR}${mods}" comes before the input filters`,
  [M.FilterNameHasModifiersOutput]: (name) => `"${name}" is not a filter name: "${MODIFIER_SEPARATOR}" cannot appear in one.`,
  [M.OneModifierList]: (prop) => `"${prop}": one modifier list`,
  [M.NoPropertyName]: (prop) => `"${prop}": the left side of a binding must name a property.`,
  [M.TooManySegments]: (p, n) => `"${p}" has ${n} path segments — the limit is ${MAX_PATH_SEGMENTS}.`,
  [M.SelectorRemoved]: (t) => `"${t}": the "@name" selector was removed in v2`,
  [M.EmptySegment]: (t) => `"${t}": the right side of a binding must name a state path (a path segment cannot be empty).`,

  [M.StructuralNotSingle]: (t) => `"${t}": if / elseif / else / for must be single binding.`,
