...
// state($stream) + sse + network: 同じ SSE フィードを <wcs-sse> タグと $stream
// (EventSource を標準 ReadableStream で包むブリッジ) の両イディオムで消費する
// ダッシュボード。$stream の実ブラウザ検証を兼ねる:
...
test.describe("examples/state-sse-dashboard", () => {
  test("両パネルにサンプルが流れ、ホスト切替で $stream 側が自動リセット・旧接続が閉じる", async ({ page }) => {
    ...
    // 右パネル($stream): ReadableStream ブリッジ経由でサンプルが fold される
    await expect(async () => {
    ...
    // 開いている SSE 接続は 2 本 (左 <wcs-sse> + 右 $stream)
    await expect(async () => {
    ...
    // $stream 側: args の依存駆動 restart で initial から数え直し (手動リセット不要)
    await expect(async () => {
