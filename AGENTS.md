...
- **The `@wcstack/state` size gates have very little headroom right now.** `scripts/check-state-size.mjs --check` allows 3 % over `scripts/state-size-baseline.json`, and the split core sits at 53,891 B gzip against a 54,025 B limit — **134 B, or 0.25 %**. (`auto.min.js` has 1.0 %, `index.esm.js` 0.8 %, and the per-feature gate in `scripts/state-split-baseline.json` is separate.) Adding almost anything to `src/core/`, `src/proxy/`, `src/bindings/` or the other core directories will fail CI. Re-record with `--update` on both scripts as part of the release, and treat a failure before then as a real signal, not as a stale baseline.
