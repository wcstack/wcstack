...
**`<wcs-ws>` はポータブルな IO ノードです。** WebSocket の接続管理・再接続・メッセージングといった IO 処理を、フレームワークに依存しない単一の Custom Element に閉じ込めています。従来、WebSocket を React で扱うには React 専用のライブラリか生の WebSocket API の二択でしたが、`<wcs-ws>` は薄いアダプター (`@wc-bindable/react`) を介して**状態購読**という統一的な形で利用できます。IO ノード自体はフレームワークを問わずポータブルであり、同じ `<wcs-ws>` が Vue でも素の HTML でもそのまま動作します。
