...
`wcs-validate` の一部ではなく単体のスクリプトにした。あの検証器は設計として
`<wcs-state>` のインラインスクリプトを正規表現で見るものでモジュール解決を持たず、
しかも辞書は**実行時ロケールで決まる動的 import** の先にあるので、`t` がどのファイル由来かは
そもそも静的に決まらない。データファイル 2 つを比べるだけなら、その機構は 1 つも要らない。
