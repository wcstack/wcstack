...
- **`<wcs-head>` 内のバインドした `<title>` が空になる。** `<wcs-head>` は子要素を
  ...
  `<title>` を残している
- **`<wcs-route>` 内のバインドが、そのルートを soft navigation で初めて開いたときだけ
  初期化されない。** ハードロードでは入り、同じルートの 2 回目でも入る。注文ページから
  About をクリックした初回だけ見出しが空になる。`e2e/tests/router-i18n.spec.ts` が
