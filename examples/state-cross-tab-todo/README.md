...
- **Durable list**: `<wcs-storage data-wcs="value#init=element: todos">` — a two-way `value` binding is the entire integration. Auto-loads on connect, auto-saves on every change, auto-syncs from other tabs. No `manual`, no trigger, no command.
...
- **`#init=element` settles load-before-bind.** `<wcs-storage>` loads and announces the persisted list in its own `connectedCallback`, which can happen *before* these bindings attach. `value` is a two-way member, so its default binding authority is `state`: the initial apply would write the `todos` seed into storage and wipe the saved list on every reload. `#init=element` makes the element the authority **for the initial sync only** — no initial write, and the already-loaded value is pulled into `todos` instead. Later assignments still flow state→element, which is why auto-save keeps working (the cross-tab test proves both halves).
